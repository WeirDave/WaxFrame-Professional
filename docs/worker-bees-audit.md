...
- 🖥 **Server Based AI** — shows server-imported customs only (Ollama, LM Studio, Open WebUI, the internal gateway)
...
| `importServerModal` | "Import from Model Server" (Server mode only) | Quick-pick UI for known model servers (Ollama, LM Studio, Open WebUI, the internal gateway) | `showImportServerModal` (app.js:8213) |
