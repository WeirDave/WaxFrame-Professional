...
async function testAllKeys() {
  ...
  console.log('[testAllKeys] passed:', passed, 'failed:', failed);
  try {
    if (failed.length === 0) {
    ...
    console.error('[testAllKeys] summary error:', summaryErr);
    alert(`Tests done — passed: ${passed.length}, failed: ${failed.length}`);
  }
}
