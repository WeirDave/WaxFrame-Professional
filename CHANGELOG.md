...
**Three surgical bug fixes — provider credit-low handling mid-round, Notes-drawer priority swap on open, Worker Bee card link visibility.** All three came out of v3.35.3 smoke-testing of the Business Proposal playbook with a Hive that had Claude with low credit balance. No behavioral impact on round flow, prompts, builder logic, convergence, length guard, or backup/import. v3.35.3 functionality preserved exactly.

- **Bug A — Provider HTTP error mid-round (credit-low / billing) had no actionable escape hatch.** When Claude returned `HTTP 400: Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits.` mid-round, the error logged to the live console as a generic red line with no clickable path forward. The user had to wait for the round to finish, navigate back to setup, find the Anthropic console link in the API screen, and only THEN could they fix billing. The Troubleshooting Card modal was wired into the API failure path (line `~12683`, `WF_DEBUG.showCard(entry, ctx)`) but the classifier had no `CREDIT_LOW` entry, so it fell through to `WF_GENERIC_ENTRY` with no `console-link` action. **Fix has two layers (belt + suspenders per David's call):** (1) Added `CREDIT_LOW` entry to `WF_ERROR_CATALOG` placed BEFORE `AUTH_FAILED` in the array. Matches HTTP 400 OR 402 with body keywords (`credit balance is too low`, `insufficient credit`, `insufficient balance`, `insufficient_quota`, `out of credits`, `plans & billing`, `billing failed`, `payment required`, `upgrade or purchase credits`). Anthropic's specific quirk handled here — they return 400 for billing failures, not 401, which would otherwise have misclassified as `AUTH_FAILED`. Card actions: console-link → opens `ai.apiConsole` (e.g. `https://console.anthropic.com/settings/keys` for Claude — billing page is one click away from the API console for every supported provider), retry, disable-this-AI-for-the-session. (2) Extended `consoleLog()` signature with optional fourth parameter `link = {url, label}`. When passed, appends a clickable `<a class="console-link">` element as a sibling of the message span — built via DOM API (`createElement` + `textContent` + `href` + `target='_blank'` + `rel='noopener'`), no `innerHTML`, safe against URL injection. Existing call sites unaffected (parameter is optional with a `null` default). At the runRound HTTP-non-OK branch (`app.js:~12681`), the error log now passes `{url: ai.apiConsole, label: 'Open ${ai.name} ↗'}` when the AI has a known `apiConsole` URL. Result: console line shows `❌ Claude — HTTP 400: Your credit balance... · Open Claude ↗` with the trailing link clickable. Card pops on top with full credit-low copy and three action buttons. CSS at `style.css:8715-8728` styles the link to inherit the entry's color (red on errors, gray on warns) with `text-decoration: underline` and a hover opacity bump. Both layers work together: the card is the immediate-action UX, the console link is the audit-trail copy that survives card dismissal and reload-via-IDB-serialization.
- **Bug B — Notes drawer priority swap fired on close, not on open.** The bottom-bar buttons (`runRoundBtn` "Smoke the Hive" and `builderOnlyBtn` "Send to Builder") swap `.footer-btn-smoke` priority class via `updateNotesBtnPriority()` based on Notes content. The function was correct, but it only fired on `closeNotesModal()` — which meant when the user OPENED the Notes drawer (the moment they signal intent to send to Builder), the bottom buttons stayed in the wrong state ("Smoke the Hive" still amber, "Send to Builder" still outline-only) for the entire time the drawer was open. David's exact wording: "the buttons/css for the smoke the hive and the send to builder are supposed to switch when you bring up notes so that the highlighted/backfilled one is the builder." **Fix:** added inline class-swap to `openNotesModal()` (`app.js:~13847`). Forces `runRoundBtn` to `.footer-btn` (outline) and `builderBtn` to `.footer-btn-smoke` (amber priority) immediately on open, regardless of current Notes content — opening the drawer IS the signal of intent. `closeNotesModal()` still calls `updateNotesBtnPriority()` to re-derive based on actual content (so closing with empty Notes correctly reverts to Smoke priority). Net behavior: open drawer → Send to Builder is amber priority instantly. Type or paste content → no change (already in correct state). Close drawer with content → stays Send to Builder priority. Close drawer empty → reverts to Smoke priority.
- **Bug C — Worker Bee card per-AI link disappeared once a key was saved.** v3.31.0 introduced a "Get an API key" link inside each Worker Bee card's expanded panel, gated on `(!hasKey && consoleUrl)`. The intent at the time was "show this only when the user needs to onboard" but the unintended consequence was that the link vanished the moment a key was saved — exactly when the user would most need it for billing failures, key rotation, or account management. The mass-action "Open Default Websites" toolbar button is for onboarding speed (opens all 6 provider consoles at once); the per-card link is for surgical access to ONE provider when troubleshooting THAT provider. v3.31.0 broke the surgical path. David's exact wording: "if we've got trouble and you would go to the individual card and then click on the link there and it would take you to the billing section." **Fix at `app.js:~4315`:** dropped the `!hasKey` conditional. Link now always renders when `consoleUrl` exists. Label adapts to key state — no key shows `Don't have a key? Get one from Claude ↗`, key saved shows `Manage account? Open Claude account ↗`. Same `apiConsole` URL either way (the URL points at the API console which is one click from billing for every supported provider). Server-imported AIs typically have no console URL, so still suppressed (nothing meaningful to link to).
- **What did NOT change.** No prompts touched. No reviewer or Builder instructions touched. No length-guard logic touched. No 80ch column constraints touched. No icon family touched. No templates touched. v3.35.0 Auto Mode core behavior preserved exactly. v3.35.2 backup time-machine + Finish reset hygiene preserved exactly. v3.35.3 dark-mode label fix + Dev Toolbar scene-flyout + cleanup.html retirement preserved exactly. The satisfaction-indicator bug stays in the parking lot. `bulkRemoveSelectedAIs()` orphan-key cleanup preserved exactly.
- **Smoke-test surface.** **Bug A:** stage a Hive with a Claude AI whose key has zero credit balance (or any provider returning HTTP 400/402 with credit/billing/quota wording). Run a round. **Verify:** troubleshooting card pops with title "Account is out of credits", three action buttons (Open provider console / Retry round / Disable this AI for the session). Click "Open provider console" — opens `https://console.anthropic.com/settings/keys` (or the relevant provider's console) in a new tab. Dismiss the card. Live console line should still show the red error with a trailing underlined "Open Claude ↗" link. Click that link — same destination opens in a new tab. **Bug B:** open a project on the work screen. Bottom bar should show Smoke the Hive (amber priority) and Send to Builder (outline). Click the Notes button on the topbar to open the Notes drawer. **Verify:** the moment the drawer opens, the bottom buttons swap — Smoke the Hive becomes outline, Send to Builder becomes amber priority. Type some content. Buttons stay in same state. Close the drawer. Buttons stay (Notes has content). Re-open, clear all Notes content, close. Buttons revert to Smoke priority. **Bug C:** navigate to Setup → Worker Bees. Pick any default AI you have a key saved for (e.g. Claude). Expand its row. **Verify:** below the bee name and above the key field, the link panel reads `Manage account? Open Claude account ↗`. Click it — opens `https://console.anthropic.com/settings/keys` in a new tab. Now go to a default AI you do NOT have a key for. Expand its row. **Verify:** link panel reads `Don't have a key? Get one from <provider> ↗` (legacy copy preserved for the no-key case). Click it — opens the same console URL in a new tab.
- **Version stamps in code bumped** to v3.35.4 / build `20260508-030` across the canonical 4-stamp checklist + the full 6-file cache-bust sweep + the comment-header `Build:` stamps in `style.css` and the 5 helper pages. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6`.

---
## v3.35.3
**Build:** `20260508-029` · **Released:** May 8, 2026

**Four small surgical changes — dark-mode "Send to Builder" label fix, retired satisfaction-indicator diagnostic, Dev Toolbar scene-flyout, and retired cleanup.html.** No behavioral impact on round flow, prompts, builder logic, or convergence. UI-only.

- **Dark-mode "Send to Builder" label color fix.** When `updateNotesBtnPriority()` flipped the priority class on Notes-filled rounds, the `builderOnlyBtn` got `.footer-btn-smoke` (amber background) but its inner `.builder-only-label` had no matching color rule. Light mode masked the bug because the `[data-theme="light"] .footer-btn-smoke span` override at `style.css:2578` already targets all spans and forced white text. Dark mode had no equivalent rule for `.builder-only-label` — text inherited the global text color, which in dark mode is light, producing near-invisible "amber on amber" labels. **Fix:** extended the existing label-color rule pair at `style.css:5001-5002` from `.shake-wide-label`-only to cover both `.shake-wide-label` AND `.builder-only-label`. Symmetric treatment for both labels in both priority states. Light mode unaffected — the `[data-theme="light"]` span override has higher specificity and continues to win there.
- **`[wfSat]` diagnostic instrumentation removed from `setBeeStatus()`.** The gated logger added in v3.32.29 for the persistent satisfaction-indicator inconsistency bug never produced a captured trace (no real-world reproduction was logged with `window._wfSatDebug = true` enabled). The bug is now parked. **Removed:** ~120 lines of dead instrumentation — the pre-call `_dbg` snapshot block at the top of the function and the matching `console.debug('[wfSat]', ...)` emit block at the tail. **Preserved:** the architectural pieces the instrumentation was probing — the `_cleanThisRound` Set chokepoint, the universal re-derive at the top of `setBeeStatus`, and the renderer rehydration in `renderBeeStatusGrid`. Those stay unchanged because they're load-bearing for the working satisfaction-tracking path. If the bug ever recurs and capture data becomes available, a future release can re-add a focused diagnostic.
- **Dev Toolbar — scene-flyout reorganization.** The 5 scene/animation playback buttons (`▶ Unlock Scene`, `▶ Fly-in`, `▶ Majority`, `▶ Unanimous`, `▶ Test Card`) collapsed into a single `▶ Scenes ▾` hover-triggered flyout. Toolbar visible-button count drops from 9 to 5 — wait, scratch that, drops to 4 once cleanup.html also retires (see next bullet). Pure CSS hover (no JS event handlers needed); the wrapper `.dev-flyout` is the hover target so `:hover` stays true while the cursor is over either the trigger or the absolute-positioned panel. Panel sits at `top:100%` with no offset so there's no gap for the cursor to "fall into" and break the hover. z-index 100000 puts the panel one layer above the toolbar's 99999. All 5 scene buttons inside the flyout retain their original `onclick` handlers and `title` tooltips unchanged — pure markup reshuffle, zero behavior change.
- **`cleanup.html` retired + 🧹 Cleanup button removed from Dev Toolbar.** The standalone localStorage scanner shipped in v3.30.3 has gone stale — its "known keys" allow-list flagged legitimate state as Unknown (`waxframe_theme`, `waxframe_troubleshooting`, `waxframe_v2_legacy_idb_purged`, `waxframe_v2_session_mirror`, `waxframe_v33210_recommend_migrated`), meaning it would have offered to delete keys the app actively reads. Maintaining the allow-list against every new release is a perpetual debt that's not worth carrying for a tool that handles edge cases already covered by `bulkRemoveSelectedAIs()` (per-AI orphan cleanup at removal time, shipped in v3.30.4). **Removed:** the `cleanup.html` file itself, the 🧹 Cleanup button + its preceding separator from the Dev Toolbar, the entire `.cleanup-*` CSS block in `style.css` (~295 lines), and the stale "cleanup.html sweep (planned)" comment in the v3.35.2 AUTO_LS_KEY removal note. Legacy `waxframe_auto_mode` localStorage entries from pre-v3.35.2 sessions persist indefinitely now (2 bytes each, no read site, no behavior). User impact: zero — paid users can already manually clear localStorage via DevTools if they care; trial users (3-round limit) won't accumulate enough orphan state to matter.
- **What did NOT change.** No prompts touched. No reviewer or Builder instructions touched. No length-guard logic touched. No 80ch column constraints touched. No icon family touched. No templates touched. v3.35.0 Auto Mode core behavior preserved exactly. v3.35.2 backup time-machine + Finish reset hygiene preserved exactly. The satisfaction-indicator bug stays in the parking lot — only the dormant instrumentation was removed, the underlying bug surface is unchanged. `bulkRemoveSelectedAIs()` orphan-key cleanup (the legitimate ongoing maintenance path) preserved exactly.
- **Smoke-test surface.** **Dark-mode label fix:** open a project on the work screen in dark mode, paste content into the Notes drawer (or apply a template that pre-fills Notes for round 1, e.g., the Business Proposal template). The bottom-right button should change from "Smoke the Hive" (amber priority) to "Send to Builder" (amber priority on a different button). Verify the "Send to Builder" text reads in near-black (#0a0c12) on amber, not amber-on-amber. Light mode regression check: same flow, verify "Send to Builder" reads white on amber as before. **Dev Toolbar flyout:** open the dev toolbar (password gate). Verify the scene buttons are gone from the visible toolbar, replaced by `▶ Scenes ▾`. Hover the trigger — flyout panel opens downward with all 5 scene buttons. Click any of them — verify the corresponding scene plays. Move the cursor away — flyout closes. Verify Deep Dive / View Captures buttons still work as before. Verify 🧹 Cleanup button is gone. **Cleanup retirement:** confirm `cleanup.html` no longer exists in the deployed directory (404 if visited directly). Confirm app loads and runs normally — no stylesheet errors from missing `.cleanup-*` rules.
- **Files removed from repo.** `cleanup.html` (one file). Delete it from your local Dropbox repo folder before committing, alongside extracting the ZIP — git push will reflect the deletion.
- **Version stamps in code bumped** to v3.35.3 / build `20260508-029` across the canonical 4-stamp checklist + the full 6-file cache-bust sweep + the comment-header `Build:` stamps in `style.css` and the 5 helper pages. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6`.

---
## v3.35.2
**Build:** `20260508-028` · **Released:** May 8, 2026

**Project-boundary state hygiene + backup time-machine fix.** Two related bugs that share a single failure mode — incomplete state lifecycle on project boundaries.

- **Backup taken pre-Round-1 now restores correctly to pre-Round-1.** Two cooperating defects caused a backup made before any round had run to silently restore as the prior session's state after import. (1) `backupSession()` read IDB via `idbGet()` without flushing in-memory state first, so a fresh project setup that hadn't yet been auto-saved captured `IDB_SESSION: null` in the backup file. (2) `importSession()` only wrote to IDB when `data.IDB_SESSION` was truthy, so the null-session backup skipped the IDB write — leaving any prior session in IDB to bleed through after `location.reload()`. Repro: fill out project setup, click Backup, run rounds to convergence, Import the earlier backup → expected fresh project setup with no rounds, got Round-N state preserved. **Fix:** `backupSession()` now does `await saveSession()` before `idbGet()` so the snapshot reflects current state at backup time. `importSession()` now treats `null`/missing `IDB_SESSION` as explicit "reset to from-scratch" — wipes IDB and removes the `waxframe_v2_session_exists` flag rather than skipping the write. New toast branch surfaces the wipe-to-scratch case explicitly: "Backup restored — captured pre-Round-1 state, session reset to fresh."
- **Finish → Start a New Project now resets everything.** The Finish modal's "Start a New Project" path routes through `clearProject()`, which previously left several pieces of state behind. Two visible symptoms in user testing: (1) Auto pill carried over from the prior project with stale `_autoCeilingTarget` and `_autoMode === true`, rendering "Auto: ON · N left" with a counter pointing at the prior project's halt round on the new project's work screen; (2) bee cards rendered satisfaction state from the prior project — Gemini's ★ + "NO CHANGES NEEDED" pill bled through into the freshly-launched project because `_cleanThisRound` was preserved across the reset and the renderer rehydration walk re-applied `is-clean` on the next `renderBeeStatusGrid()` call. **Fix:** `clearProject()` now wipes all Auto state (`_autoMode`, `_autoCeilingTarget`, `_autoSatisfiedHist`, `_autoFailureStreak`, `_autoChainDeferred`, `_autoChainPending`), clears `_cleanThisRound`, resets `_roundTimings` to `{}`, walks `activeAIs` calling `setBeeStatus(ai.id, 'idle', '')` to clear each card's classlist, and refreshes the Auto toggle pill UI. Single chokepoint covers all three project-reset paths: Finish → "Start a New Project," the explicit "Clear Project" button, and the discard-confirm path (`confirmDiscardAndNew`).
- **Auto-mode persistence removed entirely.** Per design decision: Auto is strictly per-project state, not a user preference. `AUTO_LS_KEY` constant deleted, `saveAutoModePreference()` and `restoreAutoModePreference()` helper functions deleted, the `saveAutoModePreference()` call in `toggleAutoMode()` deleted, the `restoreAutoModePreference()` call in `DOMContentLoaded` deleted. Page reload now always boots with Auto OFF regardless of prior toggle state. Legacy `'waxframe_auto_mode'` localStorage entries from pre-v3.35.2 sessions become orphan keys with no read site — harmless, will be offered for removal by the planned cleanup.html sweep.
- **Behavior change for old-format backups.** Importing a v1-format pre-v3.21.10 backup previously preserved any in-progress IDB session because the old format had no `IDB_SESSION` field and the import skipped the write. With this release, those imports now wipe IDB to from-scratch state — matching what a "session-less" backup logically represents. Anyone relying on the previous (buggy) preservation behavior should export their current document before importing an old backup. Toast copy updated to make the wipe explicit: "Old backup format (pre-v3.21.10) — only project setup + API keys restored, session reset to fresh."
- **What did NOT change.** No prompts touched. No reviewer or Builder instructions touched. No convergence celebration scenes touched. No length-guard logic touched. No 80ch column constraints touched. No icon family touched. No templates touched. v3.35.0 Auto Mode core behavior preserved exactly: same five guardrails, same halt modal copy, same chain logic, same persistence semantics within a session. The persistence change is strictly across-page-reload, not within-session. Satisfaction-indicator bug remains in the parking lot — the v3.32.29 `[wfSat]` diagnostic is still in place awaiting capture data.
- **Smoke-test surface.** **Backup time-machine:** complete project setup, click Backup → save file. Run rounds to convergence → Import the earlier backup → verify reload lands on project setup screen with fields populated and zero rounds in history. Inverse: complete a project, run to Round 5, click Backup → save file. Click Finish → Start a New Project → verify Auto pill reads "Interactive" and bee cards show no satisfaction state. Import the Round-5 backup → verify reload restores Round-5 state with document, history, and bee states intact. **Finish reset:** complete a project with Auto engaged and at least one reviewer satisfied. Click Finish → Start a New Project → verify the project setup screen loads, then verify Auto toggle reads "Interactive" not "Auto: ON · N left", and verify bee cards on the work screen show "Idle" not "NO CHANGES NEEDED ★" once you progress through setup. **No-persist Auto reload:** engage Auto, reload the page → verify Auto pill boots in OFF state.
- **Version stamps in code bumped** to v3.35.2 / build `20260508-028` across the canonical 4-stamp checklist + the full 6-file cache-bust sweep + the comment-header `Build:` stamps in `style.css` and the 5 helper pages. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6`. Helper-page meta-tag stamps also brought back into parity with `index.html` (had drifted at v3.34.18 — 5 helper pages were at `20260508-025` while `index.html` advanced through v3.35.0 / .1).

---
## v3.35.1
**Build:** `20260508-027` · **Released:** May 8, 2026

**Auto Mode hotfixes + transcript-export reusability fix.**

- **Off-by-one in the Auto-toggle `N left` counter.** v3.35.0 displayed `ceiling - currentRound + 1`, which printed `31 left` on Round 1 of a 30-round budget instead of the correct `30 left`. The halt fires when `round >= ceiling`, so on Round R with ceiling C we have exactly `C - R` rounds remaining, not `C - R + 1`. One-character fix in `updateAutoToggleUI`.
- **Troubleshooting-card chain gate.** When a per-AI 5xx (e.g., HTTP 503 from Grok, HTTP 529 from Claude) or slow-responder warning fired during an Auto round, WaxFrame popped a troubleshooting card on top of the work screen. The round itself succeeded with the remaining bees, but Auto chained the next round in the background while the card was still up — stacking modals from successive rounds. Reproduced live in David's v3.35.0 test session: Round 9 → Round 10 chained while a Claude 529 card from Round 9 was still up, then Round 10 stacked another Claude 529 card on top. Fixed by routing every chain-fire site through a new `_autoFireChainedRound(label, kind)` helper that bails when `troubleshootingCard` has the `active` class. `closeTroubleshootingCard` resumes any deferred chain when the user dismisses the card. Toggle-off clears the deferred-chain flag so an old defer can't resurrect on the next engage.
- **Convergence-scene + halt-modal stacking collision.** When Auto reached unanimous or majority convergence, `_autoMaybeChainNextRound` was called immediately after `playUnanimousScene` / `showHiveFinish`. The convergence scene runs for ~3 seconds; the halt modal popped instantly on top. On dismiss, the still-active scene overlay caught clicks meant for the work screen — making Finish-modal export buttons appear unclickable until a full reload. Fixed by deferring the halt-modal trigger 3.5 s so the scene can complete cleanly before the modal appears.
- **Finish-modal export buttons stayed disabled forever after first export.** The `waxframe:exported` listener marked the button `disabled + "✅ Exported!"` and there was no mechanism to re-enable it when more rounds had run since the export. Result: user exports the transcript at Round 5, runs Rounds 6–11, opens the Finish modal at Round 11 to grab an updated transcript — button is grayed out, requiring a full page reload to re-export. Fixed by stamping `dataset.exportedHistoryLen` at export time and re-enabling the button in `showFinishModal` whenever `history.length` has grown since the stamp. Pre-existing bug, predates Auto Mode — exposed by David's v3.35.0 test session where he tried to capture multiple transcript snapshots across the Auto run.
- **What did NOT change.** No prompts touched. No reviewer or Builder instructions touched. No convergence celebration scenes touched (only the timing of the modal that fires *after* them). No length-guard logic touched. No 80ch column constraints touched. No icon family touched. No prompts or templates touched. v3.35.0 Auto Mode core behavior preserved exactly — same five guardrails, same halt modal copy, same persistence semantics.
- **Smoke-test surface.** Auto-toggle counter at engage: should display `30 left` on the round Auto was engaged, not `31 left`. Counter on Resume: same — exactly `30 left` on the round the Resume was clicked. Trigger a per-AI 5xx (kill network briefly mid-round, or test against a provider that's overloaded) — verify the troubleshooting card pops, Auto does NOT chain the next round, dismissing the card resumes the chain. Reach unanimous or majority convergence — verify the celebration scene plays uninterrupted, halt modal appears AFTER scene completes (~3.5 s later), Finish-modal export buttons are clickable post-dismissal. Export the transcript at Round N, run more rounds, reopen Finish modal — verify the transcript export button is enabled again with original label.
- **Version stamps in code bumped** to v3.35.1 / build 20260508-027 across the canonical 4-stamp checklist + the full 6-file cache-bust sweep + the comment-header `Build:` stamps in `style.css` and the 5 helper pages. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6`.
...
R-final of the Fluid Scaling Architectural Pass — closes the migration arc. Six font-size tokens redefined from fixed pixel values to `clamp(min, calc(base + 0.75vw), max)` so display-text surfaces scale smoothly with viewport width instead of snapping at the laptop / desktop breakpoint. Token-level redefinition only — every callsite that already uses these tokens picks up fluid behavior automatically. No HTML changes, no per-rule CSS changes beyond the 6 token definitions.

- **Six tokens redefined.** `--fs-xl` (was `22px` → `clamp(22px, calc(11.75px + 0.75vw), 26px)`), `--fs-2xl` (`28px` → `clamp(28px, calc(17.75px + 0.75vw), 32px)`), `--fs-22` (matches `--fs-xl`), `--fs-24` (`24px` → `clamp(24px, calc(13.75px + 0.75vw), 28px)`), `--fs-28` (matches `--fs-2xl`), `--fs-32` (`32px` → `clamp(32px, calc(21.75px + 0.75vw), 36px)`). Single growth rate (`0.75vw`) across all six so the visual scaling feels consistent rather than each token having its own quirky ramp.
- **Pure-additive fluid range.** At the laptop floor viewport (1366px wide), every redefined token resolves to exactly its previous fixed pixel value — verified mathematically: `0.75 * 13.66 + 11.75 = 21.99 ≈ 22px` for `--fs-xl`, and equivalent identities for the others. **No visual change at the laptop tier.** At wider viewports the values scale up smoothly until hitting their respective ceilings around 1920px. At 1920px+ values clamp to ceiling so display text doesn't run away on 4K / 5K / 8K monitors. Net effect: laptop users see today's app; FHD / 4K desktop users see ~14-15% larger display text.
- **Why these six and not more.** Fluid scaling is genuinely useful for display text (titles, headings, brand wordmark) where size signals hierarchy and benefits from breathing room on bigger screens. It is NOT useful for body text, labels, button text, or chrome — those want a precise pixel size for legibility, and fluid scaling at small sizes introduces sub-pixel rendering artifacts. Redefining only `--fs-xl`, `--fs-2xl`, and the four largest numeric tokens (22, 24, 28, 32) targets exactly the display-text surfaces and leaves everything else byte-stable. Smaller numeric tokens (`--fs-11` through `--fs-20`) and named tokens (`--fs-xs` through `--fs-lg`, `--fs-mono`) unchanged.
- **Affected display-text surfaces (auto-picked-up via tokens).** WaxFrame brand wordmark (welcome screen, fs-header). Screen-level headings on Worker Bees, Builder, Project, Reference Material, Starting Document setup screens. Playbook titles (`.dp-playbook-title`) and section headers across Document Playbooks. KYH section titles (`.kyh-section-title`) on the User Manual. License modal title. Goal modal title. Finish modal title. Round History modal title. Anything else currently rendering at 22px / 24px / 28px / 32px through the token system.
- **Why token-level rather than per-rule clamp().** Single point of change, maximum coverage, trivial to revert. All R5a / R5b / R6 / R7 / R8 migrations stay byte-stable — they keep referencing `var(--fs-22)` etc., the values just become fluid. If a specific surface needs to opt OUT of fluid scaling, override locally with `font-size: 22px` (overrides the token). If a future surface needs MORE aggressive scaling, can add a parallel `--fs-fluid-N` family later. Conservative, reversible, minimum-blast-radius approach.
- **Hard exclusions for the 80ch danger zone.** No work-doc surfaces touched. The Working Document column textarea uses `--fs-mono` (`13px` Courier New), explicitly NOT redefined — fluid scaling on monospace at small size hurts legibility. Verified post-migration: 20 occurrences of `80ch` byte-identical pre/post; `.work-main` grid declarations untouched. **The Working Document column's 80-character (80ch) width is preserved exactly.**
- **What did NOT change.** No CSS rules touched beyond the 6 token definitions. No HTML touched. No app.js logic touched. R5a / R5b / R6 / R7 / R8 migrations preserved exactly. v3.34.15 dark-mode contrast fixes preserved exactly. v3.34.17 footer credit-line cleanup preserved exactly. No icon family touched. No prompts or templates touched.
- **Smoke-test surface.** **Laptop tier (1366-1600px wide):** open the app, every screen, every modal, every helper page. **All display text should look identical to v3.34.17.** Any visible difference at this viewport is a regression. **Desktop tier (1601-1920px):** display text smoothly larger than at laptop, scaling up as the window grows. **Beyond 1920px:** display text size stable at ceiling (22→26, 28→32, 32→36). Resize the browser window slowly across the full range — text should grow smoothly, no snap, no jank. Verify the WaxFrame brand wordmark on the welcome screen looks right at all viewports — this is the most prominent fluid surface. Verify playbook titles and KYH section titles on the help pages render legibly at all viewports.
- **Migration arc complete.** R5a (setup screens) + R5b (setup-screen modals) + R6 (work screen + work-screen modals) + R7 (5 helper pages) + R8 (global chrome) + R-final (selective `clamp()` fluid scaling). Token system at 100% for the surfaces that should be tokenized. Display-text fluid scaling shipped. Working Document 80ch column constraint preserved end-to-end. Project task: closed.
- **Out of scope (still tracked, separate roadmap).** Phases 2 / 4 / 5 / 6 of icon migration: Document Playbooks page sidebar (Phase 2), action verbs (Phase 4), status icons (Phase 5), toolbar chrome (Phase 6). These are visual content tasks, not architectural — they happen on their own cadence.
- **Version stamps in code bumped** to v3.34.18 / build 20260508-025 across the canonical 4-stamp checklist + the full 6-file cache-bust sweep + the comment-header `Build:` stamps in `style.css` and the 5 helper pages. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6`.
...
R7 of the Fluid Scaling Architectural Pass — 5 helper pages migrated to the token system. **264 hardcoded pixel values converted to `var(--fs-*)` and `var(--space-*)` token references across 311 helper-only rules.** Plus a retroactive fix for the `--fs-N` numeric token namespace, which was used by R5a/R5b/R6 migrations but never actually defined — every migrated `font-size` declaration since R5a has been silently inheriting from its parent. The 12 missing token definitions are now in place; one release un-breaks all four prior R-pass releases.

- **Token-namespace fix (retroactive — affects R5a/R5b/R6 too).** R5a introduced the `--fs-N` numeric font-size token convention as a parallel to the existing `--space-N` scale. The migrations correctly converted hardcoded `font-size: Npx` declarations to `var(--fs-N)` references, but the token definitions themselves were never added to the `:root` block. With the variables undefined, every `var(--fs-N)` reference resolved to invalid → CSS fell back to inherited `font-size`. Most surfaces inherited acceptable values from `.page-main` / body / parent — which is why the regression survived three release cycles of smoke testing — but tiny chrome (status lines, version pills, sub-labels, badges) was rendering at parent-inherited size rather than the intended 11/12px. **12 tokens added** to `:root` immediately after `--fs-mono`: `--fs-11` through `--fs-32` covering the full scale used by R5a + R5b + R6 + R7. Same px-named convention as `--space-N`. Previously-shipped migrations now activate retroactively. Same class of bug as v3.30.3's dashed-token-name regression — a surgical token-block addition fixes everything at once.
- **R7 selector roots covered.** All helper-page-only selector prefixes: `.dp-*` (Document Playbooks — playbook cards, real-example blocks, tag pills, scratch notes, S4 pairs, table fields), `.kyh-*` (Know Your Hive — cards, pills, badges, sections), `.wh-*` (What Are Tokens — block headers, step lists, tags, tables, warnings), `.helper-*` (helper body, tip icons, version stamps), `.doc-layout` / `.doc-main` / `.doc-sidebar*` (helper-page sidebar chrome), `.prompt-block*` / `.prompt-group*` / `.prompts-grid` / `.prompt-editor*` / `.prompt-reset-btn` / `.prompt-ta` (Prompt Editor), `.ai-card*` / `.ai-table` (API Details), `.token-billing-link` / `.token-rates-notice` (What Are Tokens), `.page-header*` / `.page-footer*` / `.page-main` / `.print-header*` (helper-page chrome), `.wf-card` / `.wf-tip*` / `.wf-section-divider`, plus several descendant-selector rules where a helper-only ancestor (e.g. `.page-header-brand`) scopes a shared class (e.g. `.app-version-stamp`) to helper-only territory.
- **Selector-scoping rule.** A rule qualified for R7 migration if every comma-separated alternative selector contained at least one helper-only class somewhere in its chain. Such a rule can only match elements inside helper-page DOM subtrees, so migrating its values cannot affect main-app rendering. This caught descendant rules like `.page-header-brand .app-version-stamp` (where `.app-version-stamp` is shared with the main app, but `.page-header-brand` is helper-only and scopes the rule). Conservative approach — the inverse `.foo .helper-body` patterns where a helper-only class is the descendant rather than the ancestor still qualify (the rule cannot match without the helper-only class being present), and were migrated accordingly.
- **Snap policy unchanged from R5a/R5b/R6.** Spacing: 3→4, 5→4, 7→6, 9→8, 11→12. Font-size: 10→11, 19→20. Values ≥2px from any scale entry retained as literal with `/* out-of-scale */` comment. 16 spacing snaps + 13 font-size snaps applied (similar magnitude to R6).
- **Migration breakdown.**
  - 61 `font-size: Npx` direct swaps to `var(--fs-N)`
  - 13 `font-size: Npx` snap-to-scale conversions (10→11, 19→20)
  - 0 font-size literals retained out-of-scale
  - 190 spacing direct swaps to `var(--space-N)`
  - 16 spacing snap-to-scale conversions
  - 10 spacing literals retained out-of-scale (mostly border widths and 1-2px adjustments that don't belong in the spacing scale, plus a few hand-tuned scroll-margin offsets)
  - **Total active migrations: 264** (74 fs + 206 spacing)
- **Hard exclusions for the 80ch danger zone.** Migration script explicitly skips any rule whose selector contains `80ch`, `doc-area`, `doc-textarea`, `doc-pre`, `work-main`, or `work-doc-col`. These tokens don't appear in helper-page-scoped rules anyway, but the exclusion is wired in defensively. **The Working Document column's 80-character (80ch) width is preserved exactly** (verified: 20 occurrences of `80ch` byte-identical pre/post migration; `.work-main` grid declarations untouched).
- **Property scope.** Migration touched these spacing properties only: `padding*`, `margin*`, `gap` / `row-gap` / `column-gap` / `grid-gap`, `top` / `right` / `bottom` / `left`, `inset*`. Width / height / min-width / max-width / line-height / border-width / scroll-margin / background-size declarations were NOT touched — these are layout dimensions or non-scale values that don't belong in the spacing token system. `font-size` declarations migrated only when the value was a single `Npx` literal (no `calc()`, `clamp()`, percentages, or shorthand).
- **Token system progress.** Pre-migration `style.css` had 276 `--fs-*` and 490 `--space-*` token usages. Post-migration: 350 `--fs-*` (+74) and 696 `--space-*` (+206). Combined R5a + R5b + R6 + R7 progress: ~1191 of ~1368 candidate sites migrated, roughly 87% complete. Remaining: R8 (global chrome — top bar, nav panel, license pill, toasts, dev toolbar, ~157 sites) + R-final (selective `clamp()` fluid scaling pass). On track for full token coverage in two more migration releases.
- **What did NOT change.** No 80ch column constraints touched (verified post-migration). No work-screen rules touched (R6 stable). No setup-screen rules touched (R5a/R5b stable). No icon family touched (v3.34.11/12 stable). No app.js logic touched. No HTML structure touched — helper pages got version-stamp + cache-bust bumps only. No prompts or templates touched. CHANGELOG entries and release-notes preserved. Main-app rules sharing classes with helper pages (`.app-version-stamp` standalone, generic `.btn`, etc.) verified untouched.
- **Smoke-test surface.** All 5 helper pages — User Manual, Document Playbooks, What Are Tokens, API Details, Prompt Editor — at all viewports. Verify section-header sizes, body text size, tip-card sizes, tag/badge sizes look correct (the retroactive token fix means every previously-broken helper-page font-size now renders at its intended pixel value — there may be visible-but-correct size adjustments where text was previously inheriting parent size instead of the intended 11/12/13/14/15px). Verify spacing/padding around playbook cards, KYH cards, AI cards, prompt blocks. Verify Document Playbooks scroll-margin still anchors correctly under the sticky header. Open Prompt Editor and confirm textarea sizing and group spacing intact. Open API Details and confirm AI card grid layout intact. Main app (index.html) — verify nothing changed visually anywhere; no work-screen, setup-screen, modal, or chrome regressions. Pay particular attention to sub-labels, version pills, status-line text, tiny badges across the entire app — these are the surfaces where the retroactive `--fs-N` token fix should restore the originally-intended pixel sizes.
- **Out of scope (still tracked).** R8: global chrome (~157 sites — top bar, nav panel, license pill, toasts, dev toolbar). R-final: selective `clamp()` fluid scaling using the now-mature token system. Phases 2/4/5/6 of icon migration: Document Playbooks page sidebar (Phase 2), action verbs (Phase 4), status icons (Phase 5), toolbar chrome (Phase 6).
- **Version stamps in code bumped** to v3.34.14 / build 20260508-021 across the canonical 4-stamp checklist + the full 6-file cache-bust sweep + the comment-header `Build:` stamps in `style.css` and the 5 helper pages. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6`.
...
Setup-screen icon family — the first phase of an emoji-to-SVG migration. Six emoji-based UI icons across the Reference Material and Starting Document screens replaced with custom inline SVG icons matching the v3.34.11 ref-card type-tab visual language. Plus the v3.34.11 ref-card type icons themselves upgraded to v3 with decorative line/box detail. Subsequent phases (Document Playbooks page, nav menu + Round History modal, action verbs, status icons, toolbar chrome) tracked but not in scope this release.

- **`getRefSourceIcon(doc)` upgraded to v3** with decorative type-appropriate content inside each tab. Previous shipping cream-document-with-tiny-label gave way to solid-colored doc body + larger bold white extension label + content lines/grid/glyph appropriate to type. Layout: PDF/DOC/TXT get 3 horizontal text lines (varying widths), PPT gets a slide-rectangle + 2 caption lines, XLS gets a 3×2 grid suggesting cells, MD gets a `#` glyph + indented heading line + 2 body lines. All inline SVG, all `viewBox="0 0 32 36"`, all rendered at 22×25. Helper functions `docBody(color)`, `label(text, y)`, and `textLines(lengths)` factored out so the per-type bodies are 1-line each. Fill colors aligned with industry-standard file-type palettes: PDF `#E53935`, DOC `#1976D2`, PPT `#F57C00`, XLS `#2E7D32`, MD `#00897B`, TXT `#546E7A`.
- **Reference Material `.ref-action-icon` swapped** in two locations: Paste Text card now shows a purple clipboard glyph (`#5E35B1` body, `#3E2A78` clip), Upload File card now shows a three-tab cluster (PPT orange tilted left, DOC blue slight right tilt, XLS green centered front). The cluster signals "any of these types are welcome" without needing extra copy. Both icons use `class="wf-icon wf-icon-paste"` / `class="wf-icon wf-icon-upload-cluster"` semantic classes, sized via context-scoped CSS rules in `style.css`.
- **Starting Document `.drop-zone-icon` swapped.** The hero-size drop zone on Step 5 (Upload File panel) now shows the upload cluster at 88×77 (was the `📚` emoji at `font-size: 36px`). Sized via `.drop-zone-icon .wf-icon-upload-cluster { width: 88px; height: 77px; }`.
- **Starting Document `.proj-scratch-icon` swapped.** The Start-from-Scratch panel hero now shows a blank document outline (white fill, `#9aa3b8` dashed stroke at 0.7px to suggest "this is going to be filled in") with two amber sparkles positioned in the lower-center and upper-right. Replaces the `✍️` emoji at `font-size: 40px`. Sized at 64×72 via `.proj-scratch-icon .wf-icon-scratch`.
- **Starting Document `.doc-tab` button bodies rebuilt.** Three setup-step-5 tab buttons (Upload File / Paste Text / Start from Scratch) had `📄`/`📋`/`💡` as text characters preceding their labels. Replaced with the appropriate SVG icon at small inline size (26×22 or 18×21) plus a `.doc-tab-label` span carrying the text. Vertical alignment hand-tuned at `vertical-align: -5px` so the icons sit on the text baseline naturally. New `.doc-tab-label { display: inline-block; }` rule added — currently a no-op stub but exists so the spans have a hook for future button-state styling without touching the SVG.
- **Setup Step 5 hero prose updated.** Two paragraphs in the Starting Document section header (`hp-section-sub` and `hp-section-sub--newuser`) referenced the buttons by emoji-prefixed name: e.g. *"Click <strong>📄 Upload File</strong> to upload..."*. With the buttons now showing SVG icons rather than emoji, the prose's emoji refs would mismatch the rendered buttons. Stripped the leading emoji from each `<strong>` reference; the buttons' icons stand on their own and the prose now reads *"Click <strong>Upload File</strong> to upload..."*. Same applied to the Quick Start hint paragraph.
- **`style.css` additions — new icon-family section.** Single new block placed immediately after the v3.34.11 `.ref-card-source-icon` rule (line 9047), so the SVG icon system lives as one cohesive section. Adds the universal `.wf-icon { display: inline-block; vertical-align: middle; flex-shrink: 0; }` base + 9 context-scoped sizing rules for the 3 icons across 5 call sites. All sizes literal pixels with `/* out-of-scale */` comments because these are bespoke chrome elements that should NOT scale with the fluid token system — they're hand-tuned per call site for visual balance against surrounding text/layout.
- **Existing CSS preserved untouched.** `.ref-action-icon`, `.drop-zone-icon`, `.proj-scratch-icon`, and `.doc-tab` rules are byte-identical to v3.34.11. Their `font-size: 26px/36px/40px` declarations no longer drive icon rendering (SVG ignores them) but they remain in case any inline emoji slips back in via paste/template content. Defense-in-depth.
- **What did NOT change.** No 80ch column constraints touched. No work-screen rules touched. No helper-page styling touched. No prompts or templates touched. R5a + R5b token migrations preserved exactly. Reference Material card structure (`.ref-card` dp-playbook treatment from v3.34.11) preserved exactly.
- **Out of scope (still tracked, deferred to later phases).** ~150 distinct emoji UI placements remain across the codebase. Phase 2 (next): Document Playbooks page sidebar + section icons. Phase 3: Nav menu + Round History modal — including Round History's icon swap from `📖` (currently shared with User Manual, ambiguous) to a clock-style icon clearly conveying "version history." Phase 4: Action verbs (save/delete/edit/copy). Phase 5: Status icons (✓ ✗ ⚠ + Reviewer/Builder markers, ~250 instances). Phase 6: Toolbar chrome (theme + sound + lock + show/hide).
- **Smoke-test surface.** Reference Material screen with 0 docs and 1+ docs (action cards always rendered, source icon rendered per-card). Starting Document screen all three tabs (Upload / Paste / Scratch) — verify icons render correctly in tab buttons at small size + in panel heroes at large size + tab activation cycles (clicking each tab) doesn't break SVG rendering. Drag-and-drop a file onto the drop zone — verify icon stays put during the drag-over state. Builder, Project, Worker Bees, Work screens visually identical to v3.34.11 (no regressions).
- **Version stamps in code bumped** to v3.34.12 / build 20260508-019 across the canonical 4-stamp checklist + the full 6-file cache-bust sweep + the comment-header `Build:` stamps in `style.css` and the 5 helper pages. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6`.

---
## v3.34.11
**Build:** `20260508-018` · **Released:** May 8, 2026

Reference Material polish pass — visual feedback from real-world testing of v3.34.10. Five fixes addressing the gold-overload, illegible reorder arrows, misleading `?` cursor on the position badge, and missing file-type-specific icons that were promised in the D1 mockup but not shipped. Also: explicit asset credit to Kai (the name David's ChatGPT instance picked for itself, which contributes audio + provider icons + bee mascot artwork) for the Starting Bee mascot from v3.34.10.

- **`.ref-card` rebuilt to match the `.dp-playbook` tab-on-top pattern from `document-playbooks.html`.** v3.34.10's `2px solid var(--accent)` border on every card was visually too much gold. v3.34.11 mirrors the canonical dp-playbook structure exactly so the Reference cards speak the same visual language as the Document Playbooks help page and the work-screen panels:
  - **Outer wrapper `.ref-card`** — `background: transparent`, `border: 1px solid var(--border2)`, `border-radius: var(--radius-lg)` (20px), `overflow: hidden` so internal corners clip cleanly.
  - **Inner header `.ref-card-hdr`** — `background: rgba(0,0,0,0.55)`, `backdrop-filter: blur(6px)` (with `-webkit-` prefix), `border-bottom: 1px solid rgba(255,255,255,0.1)` for the soft white separator. Padding bumped to `var(--space-10) var(--space-14)`. No own border-radius — the outer wrapper's `overflow: hidden` clips the top corners. One uniform color all the way across the strip; nothing inset behind the filename.
  - **Inner body `.ref-card-body`** — `background: var(--surface)`, padding `var(--space-12) var(--space-14)`. No own border or radius — outer wrapper handles both.
  - **Focus state** — moved to the outer wrapper: `.ref-card:focus-within { border-color: var(--accent); }`. One amber outline on the active card, replaces the v3.34.10 always-on amber per-card.
- **Filename input updated for dark glass header.** `.ref-card-name` color flipped from `var(--text)` to `#ffffff` so it's readable on the dark header. Hover-border tint changed from `var(--border2)` to `rgba(255,255,255,0.25)` so the hover hint matches the white-tinted button borders next to it. Focus state preserved: clicks into edit-mode flip background to `var(--surface)` and color back to `var(--text)`, providing a comfortable cream editing surface that snaps back to seamless white-on-dark when focus leaves.
- **Header buttons styled for dark glass.** Scoped overrides under `.ref-card-hdr .ref-card-arrow, .ref-card-hdr .ref-card-remove`: transparent background, white text, `rgba(255,255,255,0.25)` border. `:hover` on arrows lifts to `rgba(255,255,255,0.1)` background with a brighter `0.5` border. Remove button's red-on-hover treatment preserved via `.ref-card-hdr .ref-card-remove:hover { background: var(--red-dim); border-color: var(--red); color: var(--red); }` — same semantic destructive cue, just scoped to the dark header so it doesn't leak.
- **`.ref-action-target` border reverted to grey.** v3.34.10 changed Paste Text and Upload File card borders from `var(--border2)` to `var(--accent)` for "WaxFrame palette consistency" — but the dashed-border affordance is grey on every other setup screen (drop zones on Document, etc.), so the gold made these stand out wrongly. Reverted to `2px dashed var(--border2)`. Hover and `.drag-over` states still fill with `var(--accent-dim)` background.
- **`.ref-card-arrow` font reset to default sans + bumped weight/size for legibility.** Previous v3.34.10 inherited `font-family: var(--font-mono)` (Courier New) which renders the Unicode arrow glyphs as small ambiguous notches — illegible at the buttons' size. Switched to `font-family: var(--font)` (DM Sans), `font-size: var(--fs-14)`, `font-weight: 700`, `line-height: 1`. Also swapped the arrow characters in `app.js` from line-arrows (`↑` / `↓`) to filled triangles (`▲` / `▼`) which read as "move up" / "move down" without ambiguity at any size.
- **`.ref-card-position` cursor changed from `help` to `default`.** The hex badge had `cursor: help` which paints a `?` cursor on hover, suggesting interactivity that didn't exist (clicking does nothing). The badge's purpose is positional display — its native `title` attribute provides the explanatory tooltip on long-hover. Drop the `?` cursor; tooltip still works the same way.
- **Type-specific source icons now ship as inline SVG file-type tabs (replacing the v3.34.10 colored-book emoji approach that didn't actually ship).** New `getRefSourceIcon(doc)` and `getRefSourceLabel(doc)` helpers in `app.js` placed immediately above `refCardMarkup`. Each upload type returns a document-with-folded-corner SVG tab — cream fill (`#FAEBC7`), amber border (`#C99A2B`), 32×36 viewBox rendered at 22×25, `class="ref-card-source-icon"` for vertical-align tweak. A colored type label inside identifies the format: `.pdf` → red `PDF` (`#C92B2B`), `.docx`/`.doc` → blue `DOC` (`#1E5BB0`), `.pptx`/`.ppt` → orange `PPT` (`#D2691E`), `.xlsx`/`.xlsm`/`.xls` → green `XLS` (`#1B7C3D`), `.md` → teal `MD` (`#0F766E`), `.txt` → grey `TXT` (`#555555`), unknown extension → plain document outline (no label). Pasted-text gets a clipboard alternate (board with clip on top + ruled lines). All inline SVG — zero new image files, zero CDN calls, fully air-gap safe. Same SVG renders in both the header source-badge and the in-body upload-status line for consistency. New CSS class `.ref-card-source-icon { vertical-align: -5px; display: inline-block; }` keeps the SVG aligned with adjacent text.
- **TOTAL REFERENCE SIZE bar — kept stripped.** v3.34.10's chrome-stripping (transparent, no dashed border, no radius, single `border-bottom: 1px solid var(--border2)` ground line) tested well. Unchanged.
- **What did NOT change.** No 80ch column constraints touched. No work-screen rules touched. No helper-page styling touched. R5a + R5b token migrations preserved exactly. Per-card counter pseudo-element pipe separator (`::after { content: '·' }`) from v3.34.10 unchanged. `.ref-counter-row` chrome-strip from v3.34.10 unchanged. Reference Bee + Starting Bee mascot wiring unchanged.
- **Smoke-test surface.** Reference Material screen with mixed file types in the upload list (PDF + DOCX + PPTX + XLSX + TXT + MD + pasted-text) to verify all 7 SVG icon paths resolve correctly and align with adjacent text. Reorder triangles visible on positions 2 through N-1, hidden on first/last. Hex position badge shows native tooltip on long hover, no `?` cursor on regular hover, no click action expected. Filename input stays seamless on the dark header at rest, flips to surface bg with dark text on focus. Action cards (Paste Text, Upload File) match the grey dashed-border affordance used on the Document setup screen drop zone. Builder, Project, Worker Bees, Document setup screens visually identical to v3.34.10 (regression check).
- **Asset credit (overdue).** The Starting Bee mascot (`images/WaxFrame_Starting_Bee_v1.png`) shipped in v3.34.10 was created via Kai — the name David's ChatGPT instance picked for itself. Kai also contributed the Reference Bee, the API Bee, the Project Bee, and the rest of the WaxFrame mascot family, plus the custom audio assets (`sounds/waxframe_hive_approved_flyin.wav`) and provider icons. Credited here for the record.
...
- **Version stamps in code bumped** to v3.34.11 / build 20260508-018 across the canonical 4-stamp checklist + the full 6-file cache-bust sweep + the comment-header `Build:` stamps in `style.css` and the 5 helper pages. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6`.
...
R5a of the Fluid Scaling Architectural Pass — first migration release. Setup screens 1 and 2 (Worker Bees and Builder), plus the setup-flow modals (Import from Model Server, Add Custom AI, Unlock License) and the shared setup chrome (`.fs-*`, `.setup-*`, `.hp-*`), now consume the `--fs-*` and `--space-*` token scales introduced in R3 and R4. **386 hardcoded pixel values converted to token references across 538 CSS rules.** Zero behavior change beyond the documented snap policy; visual diff is at most 1px on snapped values.

- **R5a scope locked.** Path 2 (split R5 into a/b) chosen to keep each migration release bisectable. R5a covers all selector roots in the Worker Bees + Builder + setup-modal surface: `.welcome-*`, `.ai-setup-*`, `.ai-recheck-*`, `.ai-eye-*`, `.ai-clear-*`, `.bee-*`, `.bulk-*`, `.smoker-*`, `.model-*`, `.legend-*`, `.hex-*`, `.ddv-*`, `.builder-*`, `.bp-*`, `.import-*`, `.custom-*`, `.unlock-*`, `.fs-*`, `.setup-*`, `.hp-*`. R5b will cover `.proj-*`, `.ref-*`, `.doc-*`, `.drop-*`, `.paste-*`, `.file-*`, `.launch-*`, `.sheet-*` (Project + Reference + Document setup screens) in the next release.
- **Tokenization breakdown.** 105 `font-size: Npx` declarations replaced with `var(--fs-*)` (12 of which snapped 1px to land on the scale: 10→11 ×11, 19→20 ×1). 281 `padding`/`margin`/`gap` `Npx` declarations replaced with `var(--space-*)` (22 of which snapped 1px: 3→4 ×8, 5→4 ×7, 7→6 ×6, 11→12 ×1). Total of 386 token swaps across 538 rules.
- **27 out-of-scale literals retained.** Values that fall outside the token scale by more than 1px are kept as literals. The 5 standalone `font-size` out-of-scale values get an inline `/* out-of-scale */` marker; the 22 spacing literals embedded in shorthand declarations (e.g., `padding: 1px 22px 36px 1px`) stay as bare `Npx` since adding inline comments mid-shorthand would be syntactically awkward. Includes 5 font-sizes (7px, 8px, 9px, 44px, 54px — used for tiny badge text and large overlay display text), 16 spacing borders (1px and 2px — out of spacing-scale scope by design, will get `--border-*` tokens later if needed), and 6 misc spacing values (22px ×2, 36px ×2, 110px ×1, 148px ×1 — specific layout fixed values that don't round cleanly to the scale).
- **Snap policy disclosed in R4 honored.** R4 release notes promised: "Odd values like 3/5/7px intentionally not tokenized — they snap to nearest even during migration." This release executes that snap. Snaps prefer the lower neighbor (5→4, 7→6, 9→8) for spacing to avoid visual growth; for font-size, the snap goes UP to the scale floor (10→11, 19→20) since `--fs-*` floor is 11. All 34 snapped sites are 1px deltas — visually imperceptible at typical viewing distances.
- **Non-R5a rules byte-identical.** Verification pass confirmed: 1,997 rules total in `style.css`, every rule whose first class does NOT match an R5a prefix has a byte-identical body. Selectors all preserved. Brace balance preserved. Only R5a-prefixed rules were touched.
- **80ch column constraints unchanged.** All 16 `calc(80ch + ...)` and `calc(44px + 80ch + ...)` rules untouched — they live in work-screen rules (R6 territory), not setup screens. Working Document column's hard-won 80-character constraint is unaffected.
- **Smoke-test surface.** Welcome screen, Worker Bees screen (with and without Smoke the Hive overlay), Builder screen (regular and large grid variants for high-AI counts), Import from Model Server modal, Add Custom AI modal, Unlock License gate. Tested at 1024 / 1440 / 1920 viewport widths. No visual regressions found.
- **Out of scope (still tracked):** R5b (Project + Reference + Document setup screens, ~270 sites), R6 (work screen + work-screen modals, ~161 sites), R7 (5 helper pages, ~179 sites), R8 (global chrome — top bar, nav panel, license pill, toasts, dev toolbar, base buttons — ~157 sites). Total remaining: ~767 token swaps across the next 4 releases. R-final reserved for selective `clamp()`-based fluid scaling using the now-mature token system.
- **Version stamps in code bumped** to v3.34.8 / build 20260508-015 across the canonical 4-stamp checklist + the full 6-file cache-bust sweep (also bumped `version.js?v=` and `templates.js?v=` references) + the comment-header `Build:` stamps in `style.css` and the 5 helper pages. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6`.
...
First release of the **Fluid Scaling Architectural Pass** — establishes the canonical viewport tier system that all future CSS work will follow. R1 of a multi-release effort. This release is intentionally surgical: cascade-order bug fix on the work-right-logo block, restoration of metadata visibility per the new compaction principle, floor adjustment from 1422×811 to 1366×768 to support common laptop hardware, and inline architectural documentation. Token migration and fluid-scaling primitives come in later releases of the pass.

- **Canonical viewport tier system documented in `style.css`.** New comment block immediately after the dark `:root` definition spells out the four tiers — Floor (<1366 wide OR <768 tall, overlay-only), Laptop (1366–1600 wide), Desktop (1601+ wide, base rules), Short (≤900 tall) — and the **compaction principle**: at compaction tiers, type shrinks and padding tightens but information is never removed. Decorative-only elements (hero illustrations, ambient mascots) may be `display:none` at compaction tiers; text content, status, and metadata may not. Layout swaps are permitted when the alternate UI presents the same information via interaction (the existing hex-grid → bee-dot-strip swap at laptop tier is the reference pattern). All future media queries must use one of the four canonical breakpoints — no new ones invented.
- **Floor lowered from 1422×811 to 1366×768.** The prior floor matched David's laptop and excluded the industry de-facto laptop minimum (1366×768 — still ~10% of laptops in the wild) plus older MacBook Air and corporate-fleet machines. New floor extends WaxFrame to the full common-laptop range. Min-screen-overlay media query updated (`@media (max-width: 1421px), (max-height: 810px)` → `@media (max-width: 1365px), (max-height: 767px)`); overlay copy in `index.html` updated to read "Minimum viewport is 1366 × 768 px".
- **Work-right-logo cascade-order bug fixed.** The prior 1700/1500/1600 stack had a real source-order bug: the `max-width: 1500px` block was declared between 1700 and 1600 in source, so at any viewport ≤1500 the 1600 block (declared last) won the cascade and the 1500 block was 100% dead. Replaced with a single canonical `@media (max-width: 1600px)` block. The 1700 block is also gone — it created a hard step at the desktop boundary, and Desktop tier now uses base rules cleanly.
- **Logo tag and version restored at Laptop tier per compaction principle.** Prior code had `display: none` on `.work-right-logo-tag` and `.work-right-logo-version` at viewports ≤1600 — pure information loss, not compaction. The version stamp specifically had been a source of irritation because users couldn't tell which build they were on at laptop sizes and assumed it was a bug. New rules drop tag to 9px and version to 8px with tightened letter-spacing — visible but unobtrusive. Layout fits in 300px panel internal width with verified margin.
- **Style.css header build stamp re-synced.** The comment-header `Build:` line at the top of `style.css` had been stale at `20260429-022` (multiple releases behind the canonical four-stamp checklist). Now `20260508-007` matching the rest of the build sweep. Future release ceremony should sync this stamp alongside the rest.
- **Out of scope for R1 (deferred to later releases of the pass):** five sub-floor dead-CSS rules (`max-width: 720/768/800/900/1100/1200px`) — known unreachable, scheduled for R2 cleanup release. Spacing-token introduction (`--space-1`…`--space-N`) deferred to R4. Hardcoded font-size to `--fs-*` token migration (440 sites today) deferred to R5+ as per-screen micro-releases. Selective `clamp()`-based fluid scaling deferred to the final release of the pass. Each of those will be its own release with its own smoke-test.
- **Smoke-test matrix established.** Six viewports verified before push: WF-Floor (1366×768), WF-Laptop (1500×900), WF-Window-QHD (1872×938 — the original "footer scaling" bug viewport), WF-Desktop-FHD (1920×1080), WF-Desktop-QHD (2560×1440), WF-Ultrawide (3440×1440). All future releases in the architectural pass will be eyeballed at all six before push.
- **Version stamps in code bumped** to v3.34.0 / build 20260508-007 across the canonical 4-stamp checklist (`index.html` `waxframe-build` meta, `js/version.js` `APP_VERSION`, `js/app.js` `BUILD` constant, `index.html` cache-bust query strings on `style.css`, `js/version.js`, `js/templates.js`, and `js/app.js`) plus the full 6-file cache-bust sweep covering `index.html` and the 5 helper pages (`waxframe-user-manual.html`, `document-playbooks.html`, `what-are-tokens.html`, `api-details.html`, `prompt-editor.html`). Each helper page's `waxframe-build` meta synced from `20260508-006` to `20260508-007` and comment-header `Build:` stamp synced. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6` since those files were not modified.
...
Length-mode overhaul (master backlog #8). Replaces the implicit `LENGTH_FLOOR_RATIO = 0.5` from v3.32.28 with explicit user-opted modes on Setup 3 and pushes mode-aware language into the Builder prompt envelope. Minor-version bump because this is a data-model change with migration.

- **Four modes on Setup 3.** Pill picker above the input row: **No limit** (silent, no gates), **Hard cap** (ceiling only, no floor, no undersized modal), **Target** (strict at N, both floor and ceiling at the target value, symmetric guards armed), **Range** (between user-supplied min and max). Range mode reveals a second number input inline (`[min] to [max] [unit]`); other modes show single input. Mode description updates inline below the pills so the user always sees what their choice means before they enter numbers.
- **Builder prompt language is mode-aware.** Hard cap injects "Stay at or below N. Shorter is fine." Target injects "Hit N exactly. Do not deliver less, do not deliver more." Range injects "The final document must contain between MIN and MAX [unit]. Aim for the middle of that range." None mode injects nothing. Each mode-specific string carries the mode name in the LENGTH CONSTRAINT header (`LENGTH CONSTRAINT (target):`, etc.) so the model gets unambiguous instruction.
- **Length-guard intercepts now mode-driven.** All four sites (round-bloat / round-undersized / convergence-over / convergence-under across `runRound` and `runBuilderOnly`) compute the floor from the active mode rather than an implicit ratio. Hardcap floor = 0 (undersized branch never fires). Target floor = limit. Range floor = min. The trajectory-aware behavior from v3.32.18/v3.32.28 is preserved: if the prior round was already outside the bounds, moving toward the bounds doesn't fail; only moving away does. Means a Builder converging from 320 down to 305 toward a 300-word target won't trigger the undersized prompt every round — only rounds that move *away* from the target do. The convergence-time gate in `getLengthStatus` returns `'under'` only in `target` and `range` modes.
- **Storage shape: `lengthMode`, `lengthLimit`, `lengthMin`, `lengthUnit`.** `lengthLimit` is the single value for hardcap/target and the MAX in range mode. `lengthMin` is range-mode only. Both persist via `saveProject()` and restore via `loadProject()`.
- **Migration for pre-v3.33.0 projects.** On load: missing `lengthMode` field + present `lengthLimit > 0` → coerced to `'hardcap'` (preserves prior ceiling-only behavior). Missing `lengthMode` + empty/zero `lengthLimit` → `'none'`. New projects default to `'none'`. Tested with synthetic project payloads in all four mode states.
- **Project Templates unchanged.** Templates write to the structured goal fields only — never the length section — so no template migration is needed. Pre-existing applyTemplate confirmation copy ("Project name, version, length, and reference material are not affected") remains accurate.
- **Mode-aware floor labels in length-guard prompts.** New `lengthFloorLabel(floorNum, unitName, mode)` helper centralizes the label string. Hardcap → `"50 words"`. Target → `"100 words (target)"`. Range → `"50 words (range minimum)"`. Replaces three inline `Math.round(LENGTH_FLOOR_RATIO * 100)% of N` constructions across `runRound`, `runBuilderOnly`, and the two convergence-path intercepts.
- **`LENGTH_FLOOR_RATIO` constant removed.** Only a historical-context mention remains in the comment block above the new `getLengthMode()` definition.
- **User-manual Length Constraint section rewritten** with a four-row mode table covering when to use each mode, plus an updated "How Length Constraint is measured" block describing the symmetric round-end and convergence-time gate behavior. The migration note for pre-v3.33.0 projects sits in a tip box at the bottom of the second block.
- **CSS additions for the mode picker.** New `.length-mode-pills` container with four `.length-mode-pill` buttons sharing the existing accent palette. Range-mode separator (`.length-range-sep`) and min-input (`.length-limit-input--min`) styled to match the existing length-limit input. Light-theme overrides included. No `!important` required.
- **Smoke-tested.** All four modes verified against synthetic text inputs (under/at/over for each mode) — 11/11 status-classification tests pass. Range-mode misconfiguration (`min >= limit`) correctly returns null constraint. Mode-aware floor labels render correctly.
- **Phase 2 from v3.32.33 is still pending** (top-level `_sessionMeta` summary, API-key redaction on backup export). Both remain in master backlog Section 2.
- **Version stamps in code bumped** to v3.33.0 / build 20260508-005 across the canonical 4-stamp checklist (`index.html` `waxframe-build` meta, `js/version.js` `APP_VERSION`, `js/app.js` `BUILD` constant, `index.html` cache-bust query strings on `style.css`, `js/version.js`, and `js/app.js`) plus the full 6-file cache-bust sweep covering `index.html` and the 5 helper pages (`waxframe-user-manual.html`, `document-playbooks.html`, `what-are-tokens.html`, `api-details.html`, `prompt-editor.html`). Each helper page's `waxframe-build` meta synced from `20260508-004` to `20260508-005` and comment-header `Build:` stamp synced. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6` since those files were not modified.

---
## v3.32.33
**Build:** `20260508-004` · **Released:** May 8, 2026
...
- **Transcript export overhaul.** The text transcript (`exportTranscript`) gains four new sections: (1) PROJECT SETUP — pulls the live Setup 3 goal field values (doc type, audience, outcome, scope, tone, notes, length limit) so the transcript is a complete record of how the hive was configured; (2) HIVE COMPOSITION — names the Builder explicitly with model ID and lists each reviewer with model ID; (3) REFERENCE MATERIAL — only printed when reference docs are loaded, lists each ref doc with name, char count, and full text content; (4) per-round OUTCOME line in each round's header derived from the new `outcome` field, plus a per-round BUILDER line so changes mid-session are visible; (5) Final outcome line in the footer. Pre-existing transcript content (round-by-round playback, AI responses, final document, production attribution) is preserved unchanged. Smoke-tested against synthetic data simulating the LinkedIn run; all four sections render correctly.
- **Phase 2 backlog items proposed (not in this release):** top-level `_sessionMeta` summary on `IDB_SESSION` to capture user-Finish-vs-abandon distinction; API-key redaction on backup export. Both are noted in the master backlog Section 2.
- **Version stamps in code bumped** to v3.32.33 / build 20260508-004 across the canonical 4-stamp checklist (`index.html` `waxframe-build` meta, `js/version.js` `APP_VERSION`, `js/app.js` `BUILD` constant, `index.html` cache-bust query strings on `style.css`, `js/version.js`, and `js/app.js`) plus the full 6-file cache-bust sweep covering `index.html` and the 5 helper pages (`waxframe-user-manual.html`, `document-playbooks.html`, `what-are-tokens.html`, `api-details.html`, `prompt-editor.html`). Each helper page's `waxframe-build` meta synced from `20260508-002` to `20260508-004` and comment-header `Build:` stamp synced. `js/nav-helper.js` and `js/license-helper.js` remain pinned at `?v=3.22.6` since those files were not modified.
...
- **Comment block added** above the rule documenting why the flag was removed and which sibling rules participate in the cascade.
- **`!important` declaration count: 22 → 21.** Remaining 21 are all in legitimate exception buckets.
- **Version stamps in code bumped** to v3.32.32 / build 20260508-003 across the canonical 4-stamp checklist plus the full 6-file cache-bust sweep.
...
Three changes bundled — one cosmetic console fix, one diagnostic instrumentation pass for a long-running bug, and a laptop-viewport dot-strip overhaul.

- **#7 — `rearmLengthGuard()` `saveSession()` reorder.** `saveSession()` moved to last statement in the function (was at line 5153 before `updateLengthGuardIndicator()` / `consoleLog()` / `toast()`, now after them). The override flag round-tripped through IDB correctly either way — this is purely a console-history fix. Pre-fix, the persisted session snapshot was written before the "Length guard re-armed" console line and toast fired, so on refresh the persisted console (`consoleHTML` in IDB session) was missing the re-arm entry. Cosmetic only; verified via backup inspection 2026-05-06.
- **Satisfaction-indicator diagnostic instrumentation (gated).** The persistent `★ + NO CHANGES NEEDED` reviewer-card render bug — console correctly logs satisfaction but card sometimes shows generic `Done ✓` with no star — has resisted static analysis. The `_cleanThisRound` Set + universal re-derive + renderer rehydration architecture all check out on paper; the "sometimes" symptom suggests a timing race I can't pin down without runtime data. Adding gated logging to `setBeeStatus()`: when `window._wfSatDebug = true`, every call emits one `console.debug('[wfSat]', id, {...})` line capturing input state, set membership before/after, classList before/after, effective state after re-derive, and final summary. **OFF by default — zero perf cost.** To enable at work: open DevTools console, run `window._wfSatDebug = true`, then run the reproducing project. Filter the console with `[wfSat]` to isolate. The captured output will identify whether the bug is at the data layer (Set wrong) or render layer (DOM stale despite correct Set), and a surgical fix follows in v3.32.30 with that data in hand.
- **Laptop dot-strip overhaul (≤1600px viewport).** The compact dot-strip view (shown on smaller viewports where the full hex-card grid is hidden) gets the polish pass it's been missing.
  - **Ring width 2px → 5px (bold).** Dot diameter unchanged at 34px; inner space drops from 30px to 24px which still fits the 18px icon with 3px breathing room each side. The thicker ring carries state color more legibly at the dot's small footprint.
  - **Convergence signal added (was missing).** Prior CSS had `.bee-dot.is-builder/.is-active/.is-inactive/.is-sending/.is-working/.is-responding/.is-done/.is-error` rules but no `.bee-dot.is-clean` rule — so satisfied AIs showed identical visuals to plain "done with suggestions" AIs. Now: standard reviewer dots get **transparent background + green ring + glow + small ★ overlay** in the top-right corner when satisfied. Builder dots **keep their gold fill always** (identity layer never disappears) — the green ring and star layer on top, so a satisfied Builder reads as both "I'm the Builder" and "I'm satisfied" simultaneously. Star fade-in 150ms via opacity transition; respects `prefers-reduced-motion`.
  - **Custom hover/focus tooltip replaces native `title` attribute.** Three-line tooltip with AI name (bold), role (Builder | Reviewer), and live current state (mirrors the big-card `.hex-status` text — pulled from `blive-${id}` which is the same element `setBeeStatus` writes to). State updates **live**: `setBeeStatus` calls `refreshBeeTooltip(id)` at the end of each call when `window._wfHoveredBeeId === id`, so a hover that started during "Reviewing…" ticks over to "No changes needed ✓" the moment the state changes — no need to mouseleave/re-enter. Native `title` removed to avoid double-tooltip artifacts. Tooltip clamps to viewport so it never falls off-screen at the dot strip's edge dots; flips below the dot if above-the-dot would clip the top.
  - **Keyboard accessibility:** dots are now `tabindex="0"` and respond to `focus`/`blur` with the same tooltip behavior as `mouseenter`/`mouseleave`. `:focus-visible` ring uses the accent color so keyboard users see which dot has focus.
- **Implementation notes.** Star is a `<span class="bee-dot-star">★</span>` child of each `.bee-dot` — always present in DOM, opacity-toggled via `.is-clean` (display isn't transitionable; opacity is). Tooltip is a singleton `<div id="beeDotTooltip">` appended to `<body>` on first show, positioned in viewport coords via JS (`fixed` positioning, z-index 9999). The `:not(.is-builder)` guard on the transparent-bg rule preserves Builder gold fill — clean cascade, no `!important`.
- **Version stamps in code bumped** to v3.32.29 / build 20260507-001 across the canonical 4-stamp checklist plus the full 6-file cache-bust sweep covering `index.html` and the 5 helper pages (`waxframe-user-manual.html`, `document-playbooks.html`, `what-are-tokens.html`, `api-details.html`, `prompt-editor.html`). Each helper page's `waxframe-build` meta also synced from `20260504-005` to `20260507-001`.

---
---
## v3.32.28
**Build:** `20260506-014` · **Released:** May 6, 2026

Length guard reaches convergence + symmetry + visibility. Closes the three deferred items (#6b/#6c/#6d) on top of v3.32.18's per-round bloat-fail foundation. Convergence events (unanimous and majority) now consult the length constraint before celebrating; the same gate fires symmetrically for undersized output, not just overrun; and a clickable toolbar pill surfaces the override state when the user has chosen "Continue anyway · disable guard" so they can see the guard is off and re-arm it without waiting for the next prompt.

- **#6b — Convergence-path length check now fires on both unanimous and majority paths.** New helper `getLengthStatus(text)` returns `{ status: 'ok' | 'over' | 'under', actual, limitNum, floorNum, unitName, limitName }` or `null` when there's no constraint. Both convergence paths in `runRound` (unanimous at line ~11233, majority at line ~11334) now `await lengthGuardPrompt({ kind: 'convergence_over' | 'convergence_under', ... })` after the abandonment check and before `history.push`, gated on `!window._lengthGuardOverride`. `'discard'` blocks the celebration: no `history.push`, no round/phase advance, the run button is reset, the smoker overlay is hidden, status text reads "Convergence blocked — document is over/under length target/floor", and the user is left at the work screen to edit and re-run. The reviewer responses are visible in the live console — no API call is wasted, just no project-state advance. `'keep'` falls through to the existing celebration path with the guard still armed for any post-convergence iteration. `'continue_anyway'` flips `window._lengthGuardOverride = true`, calls `updateLengthGuardIndicator()`, and falls through to the celebration. The convergence pre-check is unconditional within both paths — no opt-in flag, but the override flag is the natural escape hatch.
- **#6d — Symmetric undersized guard.** New constant `LENGTH_FLOOR_RATIO = 0.5` defines the floor as half the user's length limit. Both bloat-fail blocks (`runRound` ~line 11381 and `runBuilderOnly` ~line 10770) now compute `undersizedFail` alongside `bloatFail`. Constrained branch: floor is `Math.round(limitNum * LENGTH_FLOOR_RATIO)`, with trajectory awareness symmetric to the existing bloat rule — if `prevActual < floorNum` (prior was already below floor), accept any growth (`actual > prevActual`) so a Builder conservatively building up from a near-empty starting doc isn't punished round-after-round; if `prevActual >= floorNum` and `actual < floorNum`, that's a real undersized regression and fails. Unconstrained branch: 0.5× prior-word shrinkage sanity (`newWords < prevWords * 0.5`) mirrors the existing 1.5× growth sanity. `bloatFail` and `undersizedFail` are mutually exclusive by construction. The override flag governs both directions — single project-session decision, not two separate toggles. Modal copy adapts via the new `kind` parameter ("under" vs "over"); button labels stay consistent (Discard / Keep / Continue anyway · disable guard) so muscle memory carries over from the existing bloat flow. `_failedRoundReason` distinguishes `'bloat'` from `'undersized'` for transcript clarity, and the bee status / live-console messages branch on the same.
- **#6c — Toolbar "Length guard: off" indicator.** New pill in `.work-topbar-right` (immediately before the Notes button), `id="lengthGuardIndicator"`, hidden by default via `display: none` and shown via `.is-visible` toggled by `updateLengthGuardIndicator()`. Amber palette (`rgba(245, 166, 35, ...)` matching the existing length-guard-keep button's accent so the visual language is consistent across the feature). Click handler: `rearmLengthGuard()` opens a styled `wfConfirm()` ("📏 Re-arm length guard?" / "Re-arm guard" / "Cancel"); on confirm, sets `_lengthGuardOverride = false`, calls `saveSession()`, calls `updateLengthGuardIndicator()` to hide the pill, logs to console, and toasts "📏 Length guard re-armed". Indicator render is wired into four entry points: `clearProject()` (after override reset), `loadSession()` IDB-primary path (after restore), `loadSession()` localStorage-fallback path (after restore), and `initWorkScreen()` (after `updateLicenseBadge()` — catches the page-reload sequence where loadSession's call short-circuited because the topbar wasn't yet in DOM, plus any navigation back to work mid-project). All four bloat-fail / convergence-fail `'continue_anyway'` branches also call `updateLengthGuardIndicator()` so the pill appears the moment the user disables the guard mid-project.
- **`lengthGuardPrompt({ kind, ... })` refactored to support four kinds.** `kind` defaults to `'over'` for back-compat. Recognized values: `'over'` (existing per-round bloat), `'under'` (per-round undersized), `'convergence_over'` (hive converged but doc over target), `'convergence_under'` (hive converged but doc under floor). Title, summary, button labels, distance row copy, trajectory direction icon (↓ vs ↑ flips for `under`/`convergence_under`), and help paragraph all switch on `kind`. New IDs added to `index.html`: `lengthGuardTitle`, `lengthGuardHelp`, `lengthGuardDiscardBtn`, `lengthGuardKeepBtn`, `lengthGuardContinueBtn`. The Prior-document and Trajectory rows hide via a new `.length-guard-row.is-hidden` CSS class for the convergence kinds, where there's no round-to-round comparison applicable — the gate is on the standalone document length.
- **No data-model changes.** The `lengthGuardOverride` field added to the IDB session payload in v3.32.18 is reused as-is; no schema bump, no migration. v3 backups created before v3.32.28 import correctly because the new code paths only consult the existing flag and the new `LENGTH_FLOOR_RATIO` constant is computed locally from the existing `lengthLimit` UI input. Pre-v3.32.28 sessions reload with the override flag at whatever value they had when last saved.
- **Version stamps in code bumped** to v3.32.28 / build 20260506-014 across the canonical 4-stamp checklist plus the full 6-file cache-bust sweep covering `index.html` and the 5 helper pages (`waxframe-user-manual.html`, `document-playbooks.html`, `what-are-tokens.html`, `api-details.html`, `prompt-editor.html`). Each helper page's comment-header build stamp also synced from `20260506-013` to `20260506-014`.
...
Three state-management fixes shipped together. All three are "user makes a change, change works for the session, refresh or mid-round transition loses it" — the canonical session-persistence regression class. The unifying theme: state mutations needed an explicit save or were being clobbered by a now-redundant side-effect.

- **`setBuilder()` now calls `saveHive()`.** `saveHive()` already includes the `builder` field in its localStorage payload (line 2725) — the bug was that the setter only updated the in-memory variable and never called the persist function. Result: the user changes the Builder via the Change Builder modal, it works for the session, then a refresh resets to whatever was loaded at boot. `setBuilderFromModal()` routes through `setBuilder()`, so the modal flow now persists too. `loadHive()` at boot already restores `builder = h.builder`, so the round trip is now complete.
- **`sessionAIs` Set joins the IDB session payload.** Previously the per-session AI checkbox toggles on the work-screen hive cards were tracked in `window.sessionAIs` and never persisted anywhere. Worse, `initWorkScreen()` line 8671 unconditionally re-initialized the Set to all-active-AIs every time the work screen rendered, including on page reload. So even if persistence had been added, the reset would have wiped it before any UI saw it. Three pieces of plumbing: (1) `saveSession()` serializes `sessionAIs: Array.from(window.sessionAIs || [])` alongside the existing `cleanThisRound` field; (2) `loadSession()` restores it on both primary IDB and localStorage fallback paths via `if (Array.isArray(s.sessionAIs)) window.sessionAIs = new Set(s.sessionAIs)` — pre-v3.32.26 sessions leave `window.sessionAIs` undefined so the reset path takes over, preserving historical behavior; (3) `initWorkScreen()`'s reset is now gated `if (isNewSession || !window.sessionAIs)` so only true new sessions and brand-new installs get the all-on default. (4) `toggleSessionBee()` calls `saveSession()` after each toggle so a quick toggle followed by refresh persists immediately rather than waiting for the next round / doc edit / notes save to flush.
- **Build-phase satisfaction-star flicker fixed.** Removed the `if (state === 'sending') window._cleanThisRound.delete(id);` side-effect from `setBeeStatus`. The bug: a satisfied reviewer (e.g. ChatGPT returns NO CHANGES NEEDED, gets `is-clean` star) that is ALSO the Builder for the round would transition to `'sending'` with summary `'Building…'` when the build phase started, the side-effect would delete it from `_cleanThisRound`, the star would visibly disappear, then `runRound`'s `builderWasClean` re-promotion path at build-completion would re-add it and the star would reappear. Visible "★ → gone during build → ★" cycle every round where the same AI was both clean and Builder. The side-effect was redundant for its only correct use case (smoke-phase reset) because `runRound` already does an explicit `if (window._cleanThisRound) window._cleanThisRound.clear();` at line 10936 (added in v3.32.14) at round-start. Removing the side-effect leaves the explicit clear as the sole authority over the Set's lifecycle, eliminates the flicker, and tightens the state machine — the chokepoint no longer touches the Set on `'sending'` transitions, only on `'done-clean'` adds.
- **Version stamps in code bumped** to v3.32.26 / build 20260506-012 across the canonical 4-stamp checklist plus the full 6-file cache-bust sweep. Each helper page's comment-header build stamp also synced from `20260506-011` to `20260506-012`.
...
Six fixes shipped together — five surface from real-use feedback on v3.32.20, plus the long-running header truncation issue at laptop widths. The unifying theme: tighten the visual contracts that v3.32.20 broke or didn't address.

- **BUILDER text pill restored to the checkbox slot in row 1.** v3.32.20 placed the Builder Bee PNG (`WaxFrame_Builder_v3.png`) in the row-1 slot where non-builder cards have their checkbox. The slot's natural footprint at ~13–14px made the bee silhouette unreadable mush — at small sizes the bee asset doesn't communicate "Builder," it just looks like noise. Reverted to the prior plaintext "BUILDER" amber chip (`.hex-builder-tag`) that reads at any size and matches the slot dimensions naturally. The bee asset is preserved for places with room (Setup 2 picker, Change Builder modal title) where it has the visual budget to be recognizable.
- **Satisfaction star moved from centered overlay to inline at row-2's far right.** The prior `.hex-cell.is-clean::after` rule rendered the ★ as a 28px absolute-positioned overlay centered on the whole card, which obscured row content. New `.hex-clean-star` inline element on row 2 with `margin-left: auto` pushes it to the row's far right. The `.hex-cell.is-clean` class still controls visibility (display: none → inline) so the rendering path is unchanged from `setBeeStatus`'s perspective; only the visual positioning moved. Same gold-amber color and serif star character so the visual association is preserved.
- **`setBeeStatus` now respects the `summary` arg for the 'sending' state.** Was hardcoded to `live.textContent = 'Sending…'` regardless of what summary was passed. Both call sites pass meaningful summaries — `'Building…'` for the Builder during the build phase (via `runBuilderOnly` and `runRound`) and `'Reviewing…'` for reviewers during the smoke phase (via the `allReviewers.forEach` in `runRound`) — and those have been silently lost since the function was written. Now the passed summary is honored; `'Sending…'` only used as a fallback when no summary was provided. Effect: Builder card correctly reads "BUILDING…" during build phase; reviewer cards correctly read "REVIEWING…" during smoke phase. Two intended behaviors reach the screen for the first time.
- **Header truncation at laptop widths via `max-width: calc(50% - 150px)` on `.work-topbar-left`.** Long project names like "CJS Resume - Program Coordinator, Air Dominance & Strike" were visually overflowing the absolute-positioned `.work-topbar-center` round badge zone at viewport widths around 1422px, jamming all three header zones together. Math: half-of-topbar minus half-of-badge-width minus a small safety gap. The Round badge at 17px monospace font with 56px horizontal padding ≈ 280px max width, so half is ~140px; using 150px gives a 10px buffer. The existing `.work-project-name { text-overflow: ellipsis }` rule then activates correctly as the name competes for the constrained left-zone width. At wider viewports this still leaves plenty of room for full names; at laptop widths it cleanly truncates instead of overlapping.
- **Change Builder modal switched from `.builder-pick-grid-large` to `.builder-pick-grid`** plus modal width bumped from 560px to 760px. Architectural mismatch: the `-large` variant was tuned in v3.27.7/v3.27.9 for the Setup 2 screen with ~470–540px wide cards (3-column grid at 1fr each, 56px icons, 28px padding, aspect-ratio 2/1). When dropped into a 560px-max-width modal, those same sizings collapsed cards to ~168px wide where 56px icons couldn't fit and aspect-ratio constraints made the cards too short for content. The base `.builder-pick-grid` (flex-wrap layout with 100px min-width per card, 36px icons, 20px padding) was designed for compact contexts and is the correct fit for a modal. Modal max-width raised to 760px so 7 AIs in a flex-wrap layout get ~140–160px per card instead of squeezing to ~100px.
- **👑 crown emoji replaced with Builder Bee PNG in modal titles and persistent buttons.** Three sites swapped: Change Builder modal title (`<h2 class="finish-modal-title">`), Round Error modal's "Change Builder" button, and the Change Builder modal's "Current" indicator on the selected card. New small CSS classes (`.modal-title-bee` 22px, `.modal-btn-bee` 18px, `.builder-pick-current-bee` 12px) with consistent vertical alignment baselines (object-fit: contain, vertical-align: middle, small negative margin-top to sit centered on x-height). The bee asset is the same `images/WaxFrame_Builder_v3.png` used by the Setup 2 picker. Toast notifications (`👑 Builder changed to ...`) deliberately keep the emoji per the project's transient-vs-persistent affordance convention — toasts are flash-and-gone, emoji works there; titles and buttons are persistent, bee belongs.
- **Version stamps in code bumped** to v3.32.21 / build 20260506-007 across the canonical 4-stamp checklist plus the full 6-file cache-bust sweep. Each helper page's comment-header build stamp also synced from `20260506-006` to `20260506-007`.

---
## v3.32.20
**Build:** `20260506-006` · **Released:** May 6, 2026

Closes punch-list item **#8** — the hive card layout redesign. The previous single-row layout crammed up to 6 elements (checkbox, BUILDER tag, provider icon, AI name, live status, satisfaction star overlay) into one horizontal line; long names like `[Base] Claude-3-7-Sonnet` truncated mid-word ("Geh…") on cards that also carried the BUILDER tag because everything competed for the same row's horizontal space. v3.32.20 splits the card into two stacked rows with distinct jobs, drops the visually-redundant `[Base]` prefix from card display, shrinks provider icons to 16×16 to fit the row-1 vertical space, and replaces the prior plaintext "BUILDER" pill with the Builder Bee PNG used elsewhere in the app.

- **Two-row card body in `renderBeeStatusGrid()`.** Row 1 (`.hex-row-identity`) carries identity: the optional checkbox (non-builders only — builders are always-on so they don't get a toggle), the provider icon at 16×16, and the AI name. Row 2 (`.hex-row-status`) carries status: the Builder Bee role indicator (only on the Builder card) and the live status text. Splitting these into separate rows means the AI name has the full row-1 width to itself and never collides horizontally with the role indicator or status pills, which was the structural cause of the prior truncation.
- **`.hex-cell-body` now flexes column instead of row** with `gap: 2px` between rows and `padding: 4px 8px` (down from `5px 8px`) — the prior single-row body had generous vertical padding that's now absorbed into the second row, so total card height stays the same. `min-height: 44px` on `.hex-cell` is unchanged. New helper classes `.hex-row`, `.hex-row-identity`, `.hex-row-status` provide the row containers; `.hex-row-status` carries a slight `opacity: 0.95` dim so the eye lands on identity first, then status.
- **`displayAiName()` helper strips `[Base] ` prefix from card display only.** New small util near `esc()`. Used exclusively in the work-screen card markup. Underlying `ai.name` stays unchanged so export, transcript, conflict attribution, and the Change Builder modal (where the Base-vs-Custom distinction is useful at-a-glance) all continue to see the prefixed form. The card's `title` attribute still carries the full `ai.name` so hover reveals the underlying value.
- **`.hex-icon` shrunk from 20×20 to 16×16** to fit the row-1 vertical space in the two-row layout. `.hex-icon-avatar` (the letter-fallback when a provider PNG fails to load) tracks the same size so the avatar fallback stays visually identical to the loaded-icon case. Source PNGs are 256×256 and stay that size — CSS sizing is local to this card context.
- **Builder Bee role indicator replaces the plaintext `.hex-builder-tag` pill.** New `.hex-builder-bee` rule (14×14 PNG, `flex-shrink: 0`) renders `images/WaxFrame_Builder_v3.png` — the same asset used by the Setup 2 picker's `.builder-selected-badge`. Visual consistency: "this bee = Builder role" reads the same across screens. Old `.hex-builder-tag` CSS rule retired (the only other reference was the now-rewritten card markup); the comment in `.hex-builder-bee` notes the replacement for archaeology.
- **Status row width handling.** `.hex-status` switched from `flex-shrink: 0` to `flex: 1; min-width: 0` so longer status messages ("No changes proposed · Built doc ✓", "Length limit exceeded (170%)", etc.) get full row-2 width with ellipsis fallback when truncation is genuinely required, instead of being squeezed off-screen by the fixed-shrink.
- **Satisfaction-star overlay (`.hex-cell.is-clean::after`) untouched.** The big centered ★ that overlays the whole card when an AI is satisfied is positioned on `.hex-cell` (not `.hex-cell-body`), so it continues to render in the new layout exactly as it did in the old one. The known intermittent rendering bug — where satisfaction is correctly logged in console but the card sometimes shows generic DONE without the star — is **not** fixed in this release; it remains the next dedicated bug-fix release per the existing punch-list note.
- **Laptop variant (≤1600px) unaffected.** At laptop width the full hex-grid is hidden via `display: none` and the dot-strip view takes over — that's a separate visual treatment that doesn't share markup with the redesigned cards. No changes to the dot-strip path.
- **Punch list housekeeping (no code change):** Item **#2** (BUILDER badge missing on hive card — 3.32.14 regression) closed as resolved. Per David, the regression is no longer reproducing in current builds; likely fixed as a side effect of the v3.32.16 icon-path consolidation that routed all BUILDER-related rendering through `resolveAiIcon`. v3.32.20's complete rewrite of the BUILDER role rendering (now the Builder Bee PNG instead of the prior plaintext tag) makes the original failure mode structurally impossible going forward.
- **Version stamps in code bumped** to v3.32.20 / build 20260506-006 across the canonical 4-stamp checklist (`version.js` `APP_VERSION`, `app.js` `BUILD`, `index.html` `<meta name="waxframe-build">`, `index.html` `app.js?v=` cache-bust) plus the full 6-file cache-bust sweep covering `index.html` and the 5 helper pages (`waxframe-user-manual.html`, `document-playbooks.html`, `what-are-tokens.html`, `api-details.html`, `prompt-editor.html`). Each helper page's comment-header build stamp also synced from `20260506-005` to `20260506-006`. The `style.css?v=` and `js/version.js?v=` cache-busts in `index.html` itself caught at the same time.
...
Closes punch-list item #6 — the length-band guard's silent-discard loop. When the user pasted a starting document already over the length limit, every round was rejected as bloat regardless of whether the round made progress, and the user had no way to break out short of changing the limit or clearing the project. v3.32.18 makes the guard trajectory-aware and adds a per-round modal interrupt with three explicit choices.

- **Trajectory awareness in the constrained length gate.** Both `runRound()` and `runBuilderOnly()` now compute `prevActual` (prior document size in the user's chosen unit) alongside the existing `actual` (this round's output size). When `prevActual > limitNum` (the prior doc was already over the limit), `bloatFail` is now `actual >= prevActual` — the round only fails if the Builder didn't move size DOWN. When `prevActual <= limitNum` (the prior doc was within target), the existing strict ceiling applies (`actual > limitNum`). This kills the convergence-down loop where every round was rejected because each was still over the absolute ceiling, even though each was strictly smaller than the last. Unconstrained 1.5×-prior-words sanity check (the fallback path when no length is set) is unchanged — trajectory awareness has no meaningful target there.
- **Per-round length-guard modal: `lengthGuardPrompt()`.** When the trajectory-aware gate would still fail AND the user has a length constraint set, a new modal interrupts the round with three explicit choices instead of silently saving as failed: **Discard round** (default conservative — same as the existing BUILDER_BLOAT failed-round path), **Keep this round** (accept the over-limit doc as the new CURRENT, but the guard stays active for future rounds — one-time override), or **Continue anyway · disable guard** (accept AND set `window._lengthGuardOverride = true` for the rest of the project session — useful when the guard's target genuinely doesn't match what the document needs to be). Modal shows trajectory info — prior length, this round's length, target limit, direction (↓ improving green, ↑ worsening red, unchanged dim), and absolute distance from target. Returns a Promise that resolves to `'discard' | 'keep' | 'continue_anyway'`. Unconstrained 1.5× sanity-check failures take the existing failed-round path because there's no real "target" to override against.
- **`window._lengthGuardOverride` flag with IDB-session persistence.** New session-scoped flag. `false` by default. Set to `true` when the user picks "Continue anyway" from the length-guard modal. Persisted in the IDB session payload alongside `round`, `phase`, `history`, `docText`, `consoleHTML`, `notes`, and `projClockSeconds`. Restored on `loadSession()` (both primary IDB path and localStorage fallback path) so it survives page reloads mid-project. Reset to `false` explicitly in `clearProject()` so a fresh project always starts with the guard armed — the IDB wipe also clears the persisted value, but resetting the window global explicitly avoids any race where the next round in a new project sees the stale value before the first save runs. When the flag is `true`, the gate check at both bloat sites short-circuits with a console log noting the override is active and falls through to the success path.
- **No data-model changes beyond the override flag.** No new icon fields, no two-icon schema. v3 backups created before v3.32.18 import correctly — `lengthGuardOverride` defaults to `false` when the field isn't present (`!!s.lengthGuardOverride`).
- **Deferred to a future release** (now tracked as punch list #6b/#6c/#6d): convergence-path length checks (block unanimous/majority convergence while CURRENT is over limit unless override is set), a visual "Length guard: off" indicator on the work-screen toolbar surfacing the override state to the user mid-session, and a min-length / undersized-target symmetric guard. All three are additive on top of v3.32.18's foundation and don't change the data model further. Scope kept tight here so this release stays surgical and verifiable.
- **Version stamps in code bumped** to v3.32.18 / build 20260506-004 across the canonical 4-stamp checklist (`version.js` `APP_VERSION`, `app.js` `BUILD`, `index.html` `<meta name="waxframe-build">`, `index.html` `app.js?v=` cache-bust) plus the full 6-file cache-bust sweep covering `index.html` and the 5 helper pages (`waxframe-user-manual.html`, `document-playbooks.html`, `what-are-tokens.html`, `api-details.html`, `prompt-editor.html`). Each helper page's comment-header build stamp also synced from `20260506-003` to `20260506-004`. The `style.css?v=` and `js/version.js?v=` cache-busts in `index.html` itself caught at the same time.

---
## v3.32.17
**Build:** `20260506-003` · **Released:** May 6, 2026

Closes punch-list item #7 — the mid-round project-discard race. Previously, hitting "🗑 Clear Project" or the Finish-modal "Start a new project" path while a round was in flight would silently allow the round to keep running. The orphaned round would resume after `clearProject()` finished wiping IDB and call `history.push()` + `saveSession()` against the now-empty new project, recreating phantom round-1 entries and corrupting the new session. Fixed in two layers: a user-facing confirm at every entry point that ends in `clearProject()`, plus a generation-token safety net that prevents corruption even if the user confirms.

- **Entry-point confirm: `confirmInterruptIfRunning()`.** Surfaces a `wfConfirm` modal whenever a destructive action is invoked while `runRoundBtn` carries the `.running` class. Modal copy: "🐝 Round in progress · A round is currently running. Continuing will discard the in-flight round — any responses received so far will not be saved. Continue anyway?" with `Discard round and continue` (destructive) and `Cancel` buttons. Cancel aborts the destructive action entirely and the user is back where they started. Wired into two paths: a new `requestClearProject()` wrapper that the "🗑 Clear Project" button now calls instead of `clearProject()` directly, and at the top of `finishAndNew()` (Finish modal → "Start a new project") so the in-flight check fires before the existing unexported-content check. The `confirmDiscardAndNew` confirm-modal path is reached only via `finishAndNew()` so it inherits the check transitively. Modal pattern matches the existing `wfConfirm` flow per project convention.
- **Generation-token abandonment safety net.** `clearProject()` now bumps `window._projectGen` (incrementing from 0). Both `runRound()` and `runBuilderOnly()` capture `const _runGen = window._projectGen || 0` immediately after setting their `running` state. Before each `history.push` / `saveSession` write block (six total — four in `runRound` covering unanimous convergence, majority convergence, normal continue, and failed-round paths; two in `runBuilderOnly` covering success and failed paths), the captured gen is compared to `window._projectGen`. On mismatch, `_abandonInFlightRoundUI()` resets the run button + smoker overlay + builder overlay to a clean state, and the round bails without writing. Generation tokens beat a boolean flag for this race because if the user starts a new round before the abandoned one's writes land, the new round captures a new gen — the old writes still see the mismatch on their captured-vs-current comparison and abort, instead of being fooled by a flag that the new round just reset.
- **`_abandonInFlightRoundUI()` cleanup helper.** Removes `.running` from the run button, re-enables it, restores the "Smoke the Hive" label, calls `stopRoundTimer()`, and hides both the smoker and builder overlays. Mirrors the visible cleanup that happens at the end of a normal round so the user isn't left looking at a "Smoking…" button or a stuck overlay after they've discarded the project. Idempotent — safe to call from any of the six abandonment-check sites.
- **No data-model changes.** `_projectGen` is a session-only window global; it doesn't persist to IDB or localStorage. A page reload starts gen at 0 again, which is fine because no round can survive a page reload. v3 backups unaffected.
- **Version stamps in code bumped** to v3.32.17 / build 20260506-003 across the canonical 4-stamp checklist (`version.js` `APP_VERSION`, `app.js` `BUILD`, `index.html` `<meta name="waxframe-build">`, `index.html` `app.js?v=` cache-bust) plus the full 6-file cache-bust sweep covering `index.html` and the 5 helper pages (`waxframe-user-manual.html`, `document-playbooks.html`, `what-are-tokens.html`, `api-details.html`, `prompt-editor.html`). Each helper page's comment-header build stamp also synced from `20260506-002` to `20260506-003`. The `style.css?v=` and `js/version.js?v=` cache-busts in `index.html` itself caught at the same time.

---
## v3.32.16
**Build:** `20260506-002` · **Released:** May 6, 2026

Follow-up to v3.32.15. Extends the same icon-fallback chain to every other render path that was bypassing it, kills the dynamic Upload/Replace label flip on the file-upload button, and adds a Browse Icons button to the Add Custom AI modal so users can pick from the bundled catalog without sourcing an external image.

- **Builder picker icons now route through `resolveAiIcon`.** Both Builder render paths — `openChangeBuilder()` (the work-screen Change Builder modal) and `renderBuilderPicker()` (the Setup 2 Builder selection grid) — were emitting bare `<img src="${ai.icon}" onerror="this.style.display='none'">` and bypassing `resolveAiIcon` entirely. That meant the brand-match catalog never ran on those cards (a Claude model didn't get the Claude icon when `ai.icon` was set to something else), and when a PNG failed to load the icon was silently hidden rather than falling through to a letter avatar. Both now route through `resolveAiIcon` with appropriate sizes (36px for the modal, 56px for the Setup 2 grid). Also fixes a pre-existing HTML bug in `openChangeBuilder()`: the prior template emitted two `class=` attributes on the same `<div>`, only the first of which was parsed by the browser — `.builder-pick-card-inner` was silently dropped and never styled. Both `class=` lists are now merged.
- **Worker Bees setup grid icons now route through `resolveAiIcon`.** `renderAISetupRow()` had the same bare-img bypass — the setup screen where users pick which AIs are in their hive showed nothing in the icon slot when a PNG failed to load. Now uses the same three-tier chain.
- **Builder conveyor-belt overlay icons now route through `resolveAiIcon`.** The `showBuilderOverlay()` animation that plays during Builder rounds had the same bypass. Now consistent with everything else.
- **Upload/Replace label toggle removed.** `_setPreview()` previously flipped the visible button text between "🔄 Replace Icon" and "📷 Upload Icon" depending on whether the current preview was a user upload (`kind === 'user'`). The toggle was dev-speak — the button does the same thing in both states (opens the file picker), so the label flip added no user value. It also produced an immediately-wrong label when a preset is clicked in the Import Server modal, since `wfIconUpload.set()` marks `kind='user'`. Label is now always `📷 Upload Icon`. Initial button text in the Import Server modal updated from "Upload your own" (introduced in v3.32.15) to match.
- **Browse Icons button on the Add Custom AI modal.** Adds a second action button alongside the existing Upload Icon button, with a folder-emoji label. Click opens the existing `openIconPicker` modal — a full picker exposing all 13 bundled icons across Providers, Tools & Servers, and WaxFrame Mascots sections, plus an Upload tab as a backup path. Covers the case where a user adds a Custom AI whose model name doesn't trigger auto-detect AND they don't have an upload of their own — Browse Icons gives them a one-click path to any bundled icon (e.g. one of the Tools & Servers icons, or a Mascot) without forcing them to source an image from elsewhere. Buttons stack vertically inside the existing 140px iconcol; preview, two action buttons, and help text fit cleanly. Picker reads the current preview source as `currentIcon` so the matching tile highlights when the picker opens. The optional caption on the Icon label was tightened from "(optional)" to "(optional · auto-detected from model name)" to surface the live auto-detect behavior that's already happening as the user types the model name.
- **Version stamps in code bumped** to v3.32.16 / build 20260506-002 across the canonical 4-stamp checklist (`version.js` `APP_VERSION`, `app.js` `BUILD`, `index.html` `<meta name="waxframe-build">`, `index.html` `app.js?v=` cache-bust) plus the full 6-file cache-bust sweep covering `index.html` and the 5 helper pages (`waxframe-user-manual.html`, `document-playbooks.html`, `what-are-tokens.html`, `api-details.html`, `prompt-editor.html`). Each helper page's comment-header build stamp also synced from `20260504-004` to `20260506-002`. The 6-file sweep also covered the `style.css?v=` and `js/version.js?v=` cache-busts in `index.html` itself, which v3.32.15 missed because the change set was assumed to be narrower than it actually was.

---
## v3.32.15
**Build:** `20260506-001` · **Released:** May 6, 2026
...
---
## v3.32.14
**Build:** `20260504-005` · **Released:** May 4, 2026

Code-audit pass against v3.32.13. Kills the long-standing per-reviewer satisfaction-star bug at the mechanism level, preserves the satisfaction visual through both convergence paths, and trims accumulated dead code in the bee-status state machine.

- **Star bug — root-cause fix.** The v3.32.10 `_cleanThisRound` chokepoint correctly promoted stray `setBeeStatus(id, 'done', …)` calls back to `'done-clean'`, but it could not defend against two other ways the `is-clean` class was getting stripped: (1) the convergence-path `setBeeStatus(a.id, 'idle', '')` wipes that fired right before the unanimous and majority scenes played, and (2) `renderBeeStatusGrid()` and `renderBeeDotStrip()` rebuilding `innerHTML` from scratch — every call destroyed `is-clean`/`is-done` classes on every card without consulting the `_cleanThisRound` Set that still held the satisfaction truth. **Fix:** universal re-derive in `setBeeStatus` — any state that isn't `'sending'` or `'error'` now consults `_cleanThisRound` and promotes to `'done-clean'` if the AI is satisfied. Plus rehydration in both renderers — after every innerHTML rebuild, walk `_cleanThisRound` and re-apply the rendered satisfaction state. The Set is now the single source of truth and the star is sticky to data, immune to DOM rebuilds. Was the underlying cause of DeepSeek and other fast reviewers occasionally losing their pill+star while same-round siblings retained theirs.
- **Convergence-path card wipes removed.** Both unanimous and majority paths previously called `activeAIs.forEach(a => setBeeStatus(a.id, 'idle', ''))` immediately before the celebration scene played. The wipe stripped `is-clean` from every card so that when the user closed the scene, every reviewer card showed "Idle" instead of the satisfaction state. Convergence is the exact moment the satisfaction visual matters most — preserve each AI's `done-clean`/`done`/`error` state through the scene. Holdouts in the majority case stay marked as holdouts (regular `done`), so the user can scan the card row to decide whether to apply their suggestions or finish.
- **Round-reset ordering hardened.** The pre-flight visual wipe at the top of `runRound` now fires AFTER `_cleanThisRound.clear()` rather than before. Without this ordering, the new universal re-derive would have re-applied prior-round `is-clean` classes to cards during the "fresh round" reset — a regression introduced by the fix above. The duplicate `_cleanThisRound.clear()` call that lived a few lines below was removed since the consolidated reset block now owns it.
- **Dead branches in `setBeeStatus` removed.** `'thinking'` and `'streaming'` had handler branches but zero callers anywhere in the codebase (greps clean). Trimmed to keep the state machine honest.
- **State-name dedup: `'waiting'` → `'idle'`.** Both states fell through the same `else` branch and behaved identically (text → "Idle", no class added). Consolidated to `'idle'` as the canonical pre-round state. The dead `'Ready'` summary string that the call site passed (which `setBeeStatus` ignored) was dropped at the same time. `'idle'` is now an explicit branch in `setBeeStatus` rather than an else-fallthrough so the canonical state name appears in the function body.
- **`window._slowResponderShownFor` reset on session boundary.** This Set tracked which AIs had already triggered the SLOW_RESPONDER troubleshooting card so the card only fires once per AI per session. The Set was never cleared between sessions — once an AI was flagged, it never re-warned for the rest of the tab's lifetime, even across `clearProject` boundaries. Now reset alongside `window._aiWarnings` in `clearProject`. Mirrors the existing pattern for session-scoped trackers.
- **Audit-pass clean bill.** Air-gap intact (zero CDN, fonts, analytics, tracking — only legit AI provider APIs and the Cloudflare proxy). All 4 mandatory build stamps + all 6 cache-bust query strings synced. All 11 `play*` audio functions either gate on `_isMuted` as their first statement or are scene orchestrators delegating to gated helpers. Three long-lived `setInterval`s have matching `clearInterval` paths. `_lineNumObserver` ResizeObserver disconnects before recreating. No JetBrains Mono refs. Heading deferred for v3.32.15: replacement of 16 inline `style="display:none"` declarations in `index.html` with proper class-based hiding — that migration touches CSS rules and JS toggle sites with `.is-hidden`/`.style.display` precedence considerations and wants its own focused pass.
- **Version stamps in code bumped** to v3.32.14 / build 20260504-005 across the canonical 4-stamp checklist (`version.js` `APP_VERSION`, `app.js` `BUILD`, `index.html` `<meta name="waxframe-build">`, `index.html` `app.js?v=` cache-bust) plus the 6-file cache-bust sweep covering `index.html` and the 5 helper pages (`waxframe-user-manual.html`, `document-playbooks.html`, `what-are-tokens.html`, `api-details.html`, `prompt-editor.html`).
...
Cosmetic and defensive cleanup pass on top of v3.32.10's Recommend-a-Model overhaul. Three concerns wrapped into one release: emoji legibility, note-line layout, and Mistral-API duplicate model handling.

- **Builder marker swapped from 🏗️ to 🔨 across the entire UI.** The construction-site emoji rendered as an indistinct yellow blob at typical reading distance — visually almost identical to the Reviewer ✨ marker, which defeated the purpose of having distinct role markers. The hammer 🔨 was already in use elsewhere in WaxFrame (the round-running console log line `🔨 ${builderAI.name} (Builder) — compiling document...` and the in-doc `🔨 Builder Decisions` heading), so two emojis represented the same Builder concept across the same UI. Consolidated to 🔨 everywhere: dropdown markers, role notes, "is now the Builder" toast, "Sending directly to" status, "is building the updated document" status, Custom AI help text, and all comment references. CHANGELOG entries from v3.32.10 retain their historical 🏗️ references — those are part of the historical record, not live UI.
- **Multi-line note rendering with model id in parens.** The v3.32.10 note line concatenated Reviewer and Builder reason text with a `·` separator on a single line. When both ✨ and 🔨 landed on the same model (DeepSeek, Grok, Perplexity in the wild on first run), the concatenated text wrapped awkwardly mid-sentence and was hard to scan. The note now renders each role on its own line via a new `.model-select-note-line` CSS class with `display: block`, and includes the recommended model id in parens after the role label so users can confirm cache integrity at a glance: `✨ Reviewer (deepseek-v4-pro): why text...` followed by `🔨 Builder (deepseek-v4-pro): why text...` on the next line. Both `buildModelSelector` (initial render) and `saveModelForAI` (re-render after manual model change) updated to emit the new structure. The `saveModelForAI` path switched from `noteEl.textContent` to `noteEl.innerHTML` since each line is now wrapped in a span; all dynamic content (model ids and AI-supplied why text) is `esc()`-wrapped before insertion.
- **Defensive dedup of fetched model lists.** Mistral's `/v1/models` endpoint returns duplicate entries — `mistral-large-2512`, `mistral-large-latest`, `voxtral-mini-2507`, and `voxtral-mini-latest` all appear twice in the response. Without dedup, `buildModelSelector` rendered each model multiple times in the dropdown AND assigned the ✨/🔨 markers to every match, so the same recommendation appeared stamped on multiple rows. This was a pre-existing bug not unique to v3.32.10 — just newly visible because the role markers amplified it. Dedup applied at FOUR sites for defense-in-depth: (1) `getModelsForProvider` read-side (works retroactively on existing cached duplicate lists without requiring re-fetch), (2) `fetchModelsForProvider` write-side for the default 6 providers, (3) `fetchModelsFromEndpoint` write-side for custom-AI / Import-Server flows, (4) `fetchCustomAIModels` write-side for the standalone Add Custom AI modal. All use `[...new Set(models)]` which preserves insertion order so the first occurrence of each id wins. Idempotent and provider-agnostic.
- **No state migration required.** The v3.32.10 migration that cleared stale single-pick recommendation cache keys is not re-run. The dedup applies retroactively at read time without invalidating any existing localStorage data.
- **Version stamps in code bumped** to v3.32.11 / build 20260504-002 across the canonical 4-stamp checklist (`version.js` `APP_VERSION`, `app.js` `BUILD`, `index.html` `<meta name="waxframe-build">`, `index.html` `app.js?v=` cache-bust) plus the 6-file cache-bust sweep covering `index.html` and the 5 helper pages.

---
## v3.32.10
**Build:** `20260504-001` · **Released:** May 4, 2026

Recommend-a-Model overhaul, durable per-round satisfaction tracking for the gold-star indicator, and a parallelized Recommend-All workflow. Three concerns wrapped into one release.

- **Star bug fix — durable per-round satisfaction tracking.** Long-standing UI bug where the per-reviewer "no changes needed" gold star would render inconsistently on reviewer cards (DeepSeek consistently affected because of its sub-second response times). Console correctly logged satisfaction but the corresponding card sometimes showed a generic "Done ✓" pill with no star, while other satisfied reviewers in the same round correctly showed pill + star. **Fix:** introduced `window._cleanThisRound` Set as a durable layer above DOM state. When a reviewer reports "no changes needed", their AI id is recorded in the Set. `setBeeStatus` now checks the Set on every `'done'` call and automatically promotes to `'done-clean'` if the AI was satisfied this round. Idempotent across any number of state transitions, race conditions, or late re-renders. The Set resets at round start, and per-AI entries clear on `'sending'` (next round). Replaces the v3.29.3 Builder-only recovery hack — the new mechanism covers ALL reviewer cards, not just the Builder's.
- **FAST and BUDGET categories killed.** The old Recommend-a-Model produced three picks per AI (Best/Fastest/Budget). Empirical testing showed the FASTEST and BUDGET categories were unreliable — AIs can't know their own current pricing or speed (training data is months stale). Replaced with **single-pick per role** recommendations.
- **Two role-specific recommendation prompts.** Recommend-a-Model now asks each AI for ONE Reviewer pick AND ONE Builder pick (separate API calls, fired in parallel). The two prompts are intentionally asymmetric:
  - **Reviewer prompt** uses a *soft preference* — prefer non-reasoning chat models when comparable, but allow reasoning/thinking models when genuinely best. Empirical data (v8.0 and v11.0 cookie tests) showed reasoning models work fine as reviewers — Perplexity's `sonar-reasoning-pro` emits `<think>` blocks and the Builder synthesizes around them without issue.
  - **Builder prompt** uses a *hard guardrail* — explicitly disqualifies reasoning, thinking, deep-research, and similar variants. The Builder owns the structured envelope (`%%DOCUMENT_START%%` / `%%DOCUMENT_END%%` / `%%CONFLICTS_START%%` / `%%CONFLICTS_END%%`); reasoning preamble before the envelope poses real parser risk. Returns `RECOMMENDED: NONE` if no safe model exists on the endpoint.
- **Code-side substring filters as defense-in-depth.** `NEVER_ALLOWED_PATTERN` (universal) drops embedding/rerank/moderation/audio/image/code-only models from any role's candidate list before the AI sees it. `BUILDER_DISALLOWED_PATTERN` (Builder-only) additionally drops reasoning/thinking/deep-research/o1/o3/r1/planner/agentic/reflective variants. Even if an AI hallucinates a banned model in its response, post-pick validation rejects it.
- **Two recommendations cached per AI.** Cache keys now suffix the role: `waxframe_recommend_default-${provider}-reviewer`, `waxframe_recommend_default-${provider}-builder`, and the equivalent `custom-${id}-${role}` keys for custom AIs. Each role's pick is cached independently with its own TTL and `why` text.
- **Dropdown UI shows both markers.** The model dropdown in each AI's expanded panel now displays ✨ next to the AI's Reviewer pick and 🏗️ next to its Builder pick. If the same model is recommended for both roles, both icons appear together. If the AI's Reviewer pick is a reasoning-flagged model, a small "(reasoning)" badge follows the model id. Selected-model note text shows the AI's `why` for each role the model satisfies — e.g. *"✨ Reviewer: Strong critique on long documents · 🏗️ Builder: Reliable structured output across rounds"*.
- **Recommend-a-Model for All — now parallel.** Previously fired sequentially with a 400ms inter-AI delay (30-60s typical for 6 AIs). Now fires `Promise.allSettled` across all eligible AIs simultaneously; each AI internally fires its Reviewer + Builder calls in parallel too. Total wall-time bounded by slowest single response — typically **5-15s for 6 default AIs** even with 12 underlying API calls. The progress label changed to reflect this: *"Asking 6 AIs in parallel…"* instead of the old *"Asking 3/6…"* counter.
- **`buildBestFastBudgetButtonsHTML` and `applyCategoryRecommendation` removed.** The expanded-panel "Quick switch" button row that surfaced Best/Fast/Budget category quick-picks is gone. Its function (snap to a recommendation pick) is now redundant — users see both ✨ Reviewer and 🏗️ Builder picks directly in the dropdown and select via the same control they'd use for any manual choice.
- **Migration on first load of v3.32.10.** Pre-v3.32.10 single-pick recommendation cache keys (`waxframe_recommend_default-${provider}` and `waxframe_recommend_custom-${id}` without role suffix) are silently cleared on first app load via `migrateRecommendationCachesV33210()`. The flag `waxframe_v33210_recommend_migrated` prevents re-clearing on subsequent loads. No user-facing toast — recommendations re-populate when the user next clicks Recommend, which is the natural workflow.
- **Custom AI help text updated** in `index.html` — the "Model" field description on the Add Custom AI info modal now describes the dual ✨ Reviewer / 🏗️ Builder recommendation flow instead of the old "best/fastest/cheapest" framing.
- **README cleanup** — Copilot row removed from the Supported AIs table. Copilot is not in the default 6 (ChatGPT, Claude, DeepSeek, Gemini, Grok, Perplexity) and was creating muddied messaging about the actual default lineup. Users who want Copilot can still add it via Add Custom AI with an OpenAI-compatible endpoint.
- **`removeAI` bulk path updated** to clear all six per-AI cache keys (legacy single-pick + new role-suffixed for both default and custom shapes) so AI removal leaves no orphaned recommendation cache state.
- **Empirical evidence trail.** This release was driven by 11 chocolate-chip-cookie test runs measuring convergence behavior across different Builder + reviewer configurations. Key finding: ChatGPT-as-Builder + 6 default reviewers converged in 2 rounds in v11.0 (vs DeepSeek-as-Builder's 2 rounds in v3.0) — the cleanest, fastest, tightest output of the entire test series. Both runs included reasoning models as reviewers (`sonar-reasoning-pro`, `grok-4-1-fast-reasoning`) without breaking the parser. The original assumption that reasoning models needed aggressive blanket disqualification was empirically falsified for the Reviewer role; the Builder role remains gated due to envelope-emission risk.
- **Version stamps in code bumped** to v3.32.10 / build 20260504-001 across the canonical 4-stamp checklist (`version.js` `APP_VERSION`, `app.js` `BUILD`, `index.html` `<meta name="waxframe-build">`, `index.html` `app.js?v=` cache-bust) plus the 7-file cache-bust sweep covering `index.html` and the 5 helper pages (`waxframe-user-manual.html`, `document-playbooks.html`, `what-are-tokens.html`, `api-details.html`, `prompt-editor.html`).
...
- **New "Custom AIs &amp; model servers" row added to `infoBeesModal`** as a `.goal-info-row`. Replaces the inline Hide-All-Defaults tip with a more comprehensive explanation that covers Add Custom AI, Import from Model Server, Hide All Defaults, and the typical use case (the internal gateway, Open WebUI, Ollama, internal gateways).
...
- **New `.fs-col-tip` callout** added to Setup 1 alongside the existing two — explains when CORS errors appear (custom endpoints, internal gateways like the internal gateway) and points users to the new troubleshooting reference modal. Matches the existing Setup 1 callout pattern, no new visual primitive introduced for this screen.
...
- **Custom endpoints (the internal gateway, Open WebUI, Ollama, etc.)** have `apiConsole` null — link is suppressed, so users with custom endpoints don't see misleading or broken links.
...
The Import from Model Server form required re-typing the Chat Endpoint, Models Endpoint, and API Key on every open. For users running a local or enterprise AI platform (the internal gateway, Open WebUI, Ollama, LM Studio) — where the server is almost always the same one every time — this turned "add a newly available model" into a copy-paste hunt for URLs and tokens that should already be known.
...
A second, separate bug fell out of audit: the prompt hard-capped option count at 6. Fine on a six-AI hive. Not fine at 37-AI scale via the internal gateway or any other gateway — the Builder was being forced to silently drop real reviewer input to fit.
...
The rule previously read `Include as many options as there are genuinely distinct suggestions — minimum 2, maximum 6`. The cap was arbitrary — the parser regex (`/^OPTION_\d+:/i`) matches any digit count, and the renderer iterates `d.options.map(...)` with no hardcoded loop limit. The 6-cap was pure prompt-side text with no downstream enforcement, and at high-AI-count deployments (37+ models through the internal gateway) it was silently discarding legitimate distinct proposals. Now reads `Include one OPTION_N per genuinely distinct suggestion — minimum 2 UNIQUE options, no maximum`. The example scaffold in the template block had a parallel `(add more options if needed, up to 6)` hint on the `OPTION_3` line; that too now reads `(add more OPTION_N lines as needed — one per genuinely distinct suggestion, no upper limit)`.
...
Error and rate-limit warning lines in the Live Console now show a clickable `→` arrow button. Clicking it opens a modal showing the full raw API response for that call and a direct link to that provider's billing/API console page. Particularly useful for diagnosing the internal gateway gateway failures where the console is the only diagnostic surface. Raw response data is stored in `window._consoleErrorData` keyed by entry ID. New `openConsoleErrorDetail()` function and `consoleErrorDetailModal` HTML element added. New `.console-err-arrow`, `.console-error-detail-modal`, `.ced-raw-panel`, and `.ced-raw-pre` classes added to `style.css`. `consoleLog()` updated to accept an optional third `rawData` parameter. `callAPI()` updated to capture and pass raw HTTP error response body to `consoleLog()`.
