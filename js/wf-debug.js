...
window.WF_DEBUG = {
  ...
  // ── Failure-record scrubbing (v3.63.500) ──────────────────────
  //
...
[…truncated ${(rec[k].length - 4000).toLocaleString()} chars for bundle export]`;
  ...
  async bundleForScout() {
    ...
    const envelope = {
      ...
      // v3.63.500 — the ring buffer is written by captureRound, which only
...
window.WF_ERROR_CATALOG = [
  ...
  {
    // v3.63.500 — Placed BEFORE PROVIDER_DOWN because classify() returns the
  ...
  {
    // v3.63.500 — Split out of BUILDER_TRUNCATED, which v3.63.489 had made
...
function renderTroubleshootingCard(entry, ctx) {
  ...
  // v3.63.500 — the label is no longer always true. This block was built for
