...
// The pre-v3.63.503 blind spot: this response HAS a conflicts block, so the
...
// v3.63.503 — the budget now comes from the model, not a constant. With no
...
// ── Requested output budget (v3.63.503) ─────────────────────────────
...
// ── Streaming SSE accumulator (v3.63.503) ──────────────────────
